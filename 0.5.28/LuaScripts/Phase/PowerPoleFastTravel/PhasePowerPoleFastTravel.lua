...
PhasePowerPoleFastTravel.s_messages = HL.StaticField(HL.Table) << { [MessageConst.TRAVEL_POLE_ENTER_TRAVEL_MODE] = { '_OnEnterPowerPoleFastTravelMode', false }, }
...
PhasePowerPoleFastTravel._DoPhaseTransitionBackToTop = HL.Override(HL.Boolean, HL.Opt(HL.Table)) << function(self, fastMode, args)
end
PhasePowerPoleFastTravel._OnActivated = HL.Override() << function(self)
    ...
    GameInstance.world.gameMechManager.travelPoleBrain:OnPanelOpened(self.arg[1])
end
...
PhasePowerPoleFastTravel._OnEnterPowerPoleFastTravelMode = HL.StaticMethod(HL.Table) << function(args)
    PhaseManager:OpenPhase(PhaseId.PowerPoleFastTravel, args)
end
