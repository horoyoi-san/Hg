...
local WEAPON_EXHIBIT_PAGE_TYPE_2_PANEL_ID = { [UIConst.WEAPON_EXHIBIT_PAGE_TYPE.OVERVIEW] = { PanelId.WeaponExhibitOverview }, [UIConst.WEAPON_EXHIBIT_PAGE_TYPE.UPGRADE] = { PanelId.WeaponExhibitUpgrade }, [UIConst.WEAPON_EXHIBIT_PAGE_TYPE.GEM] = { PanelId.WeaponExhibitGemCard, PanelId.WeaponExhibitGem, }, [UIConst.WEAPON_EXHIBIT_PAGE_TYPE.DOCUMENT] = { PanelId.WeaponExhibitDocument }, [UIConst.WEAPON_EXHIBIT_PAGE_TYPE.POTENTIAL] = { PanelId.WeaponExhibitPotential }, }
local WEAPON_DESC_SHOW_CFG = { [UIConst.WEAPON_EXHIBIT_PAGE_TYPE.UPGRADE] = false, [UIConst.WEAPON_EXHIBIT_PAGE_TYPE.GEM] = false, [UIConst.WEAPON_EXHIBIT_PAGE_TYPE.POTENTIAL] = false, }
local HIDE_GRID_PAGE_TYPE = { [UIConst.WEAPON_EXHIBIT_PAGE_TYPE.UPGRADE] = true, }
local PHASE_ITEMS = { PHASE_WEAPON_INFO_GAME_OBJECT, }
...
PhaseWeaponInfo.s_messages = HL.StaticField(HL.Table) << { [MessageConst.WEAPON_EXHIBIT_PAGE_CHANGE] = { 'OnSelectPageChange', true }, [MessageConst.ON_GEM_ATTACH] = { "OnGemAttach", true }, [MessageConst.ON_GEM_DETACH] = { 'OnGemDetach', true }, [MessageConst.ON_WEAPON_REFINE] = { 'OnWeaponRefine', true }, [MessageConst.P_WEAPON_EXHIBIT_BLEND_EXIT] = { '_BlendExitPhase' }, }
...
PhaseWeaponInfo.PrepareTransition = HL.Override(HL.Number, HL.Boolean, HL.Opt(HL.Number)) << function(self, transitionType, fastMode, anotherPhaseId)
    ...
    if transitionType == PhaseConst.EPhaseState.TransitionIn then
        ...
        UIManager:PreloadPanelAsset(PanelId.WeaponExhibitEmpty)
    end
end
PhaseWeaponInfo.OnSelectPageChange = HL.Method(HL.Table) << function(self, arg)
    ...
    self.m_effectCor = PhaseManager:_ClearCoroutine(self.m_effectCor)
    self.m_effectCor = PhaseManager:_StartCoroutine(function()
        ...
        for _, panelId in pairs(neededPanels) do
            if not self.m_panel2Item[panelId] then
                self:CreatePhasePanelItem(panelId, { pageType = pageType, phase = self, weaponInfo = { weaponTemplateId = self.m_weaponExhibitInfo.weaponInst.templateId, weaponInstId = self.m_weaponExhibitInfo.weaponInst.instId, }, isFocusJump = arg.isFocusJump })
            else
...
PhaseWeaponInfo._OnActivated = HL.Override() << function(self)
    ...
    self:OnSelectPageChange({ pageType = pageType, isFocusJump = arg.isFocusJump })
...
PhaseWeaponInfo._RefreshWeaponEquipped = HL.Method(HL.Table) << function(self, weaponExhibitInfo)
    ...
    if isWeaponEquipped then
        ...
        local sprite = self:LoadSprite(UIConst.UI_SPRITE_CHAR_INFO, UIConst.UI_CHAR_INFO_CHAR_BG_PREFIX .. charInfo.templateId)
...
PhaseWeaponInfo._RefreshWeaponDecoEffect = HL.Method(HL.Userdata, HL.Number) << function(self, weaponGo, weaponInstId)
    local _, gemColor, brightness = CS.Beyond.Gameplay.WeaponUtil.TryGetWeaponEmissionParamByInstId(Utils.getCurrentScope(), weaponInstId);
    CS.Beyond.Gameplay.WeaponUtil.SetWeaponEmissionParam(weaponGo, gemColor, brightness)
