...
CommonToggle.InitCommonToggle = HL.Method(HL.Function, HL.Boolean, HL.Opt(HL.Boolean, HL.String)) << function(self, action, value, notCall, labels)
    self:_FirstTimeInit()
...
CommonToggle._UpdateAnimation = HL.Method(HL.Boolean, HL.Opt(HL.Boolean)) << function(self, isOn, jumpToEnd)
    ...
    self.view.animation:Play(name)
    if jumpToEnd then
        self.view.animation:get_Item(name).time = math.maxinteger
    end
end
