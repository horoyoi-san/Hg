...
local MFR_STATUS_BAR_CONST = { ANIMATOR_STATE_NAME = "state", ANIMATOR_STATE_VAL = { LOCKED = 0, NOT_BUILD = 1, WORKING = 2, CAN_COLLECT = 3, STOPPED = 4, IDLE = 5, }, }
...
SSManufacturingStatusBar.SetupView = HL.Override() << function(self)
    local hasValue
    local roomInfo
    hasValue, roomInfo = Tables.spaceshipRoomInsTable:TryGetValue(self.m_roomId)
    if hasValue and roomInfo then
        local roomTitle = roomInfo.name
