...
ShopItem.InitShopItem = HL.Method(HL.Table) << function(self, info)
    self:_FirstTimeInit()
    self.m_info = info
    ...
    self.view.icon.sprite = self:LoadSprite(UIConst.UI_SPRITE_ITEM_BIG, itemData.iconId)
    ...
    if ResourceManager.CheckExists(resPath) then
        ...
        if tryIcon2 then
            ...
            self.view.moneyIcon.sprite = self:LoadSprite(UIConst.UI_SPRITE_WALLET, moneyItemData.iconId)
        end
    else
        self.view.moneyIcon.sprite = self:LoadSprite(UIConst.UI_SPRITE_WALLET, moneyItemData.iconId)
    end
