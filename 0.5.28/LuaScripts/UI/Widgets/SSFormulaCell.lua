...
SSFormulaCell._TryUpdateManufacturingFormulaCell = HL.Method() << function(self)
    ...
    local rate = GameInstance.player.spaceship:GetRoomProduceRate(self.m_info.roomId, GEnums.SpaceshipRoomAttrType.RoomProduceRate)
    ...
    node.timeTxt.text = UIUtils.getLeftTimeToSecondFull(totalProgress / rate)
...
SSFormulaCell._TryUpdateSowFormulaCell = HL.Method() << function(self)
    ...
    node.timeTxt.text = UIUtils.getLeftTimeToSecondFull(time)
...
SSFormulaCell._UpdateItemCount = HL.Method(HL.Boolean) << function(self, isLack)
    ...
    local colorStr = isLack and SpaceshipConst.FORMULA_CELL_OWN_COUNT_COLOR_STR[2] or SpaceshipConst.FORMULA_CELL_OWN_COUNT_COLOR_STR[1]
