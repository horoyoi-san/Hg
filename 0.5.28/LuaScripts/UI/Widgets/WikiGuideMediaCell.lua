...
WikiGuideMediaCell.InitWikiGuideMediaCell = HL.Method(HL.String) << function(self, wikiTutorialPageId)
    ...
    if isImg then
        self.view.image.sprite = self:LoadSprite(UIConst.UI_SPRITE_GUIDE, pageData.image)
    else
        local success, file = CS.Beyond.Gameplay.View.VideoManager.TryGetVideoPlayFullPath(string.format(GUIDE_VIDEO_PATH_FORMAT, pageData.video))
        if success then
