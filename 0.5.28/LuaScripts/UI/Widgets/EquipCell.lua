...
EquipCell.InitEquipCell = HL.Method(HL.Any, HL.Opt(HL.Function)) << function(self, equip, onClick)
    ...
    self.view.midLeft.imagePart.sprite = self:LoadSprite(UIConst.UI_SPRITE_EQUIP_PART_ICON, spriteName)
    if charInfo then
        local charId = charInfo.charId
        local charSpriteName = UIConst.UI_CHAR_HEAD_PREFIX .. charId
        self.view.imageChar.sprite = self:LoadSprite(UIConst.UI_SPRITE_CHAR_HEAD, charSpriteName)
