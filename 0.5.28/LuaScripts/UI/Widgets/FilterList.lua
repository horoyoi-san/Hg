...
FilterList.InitFilterListWithTags = HL.Method(HL.Table, HL.Function, HL.Opt(HL.Table)) << function(self, tags, onConfirm, selectedTags)
    local tagGroups = {}
    table.insert(tagGroups, { tags = tags })
...
FilterList._OnEnable = HL.Override() << function(self)
    ...
    Notify(MessageConst.SHOW_AS_CONTROLLER_SMALL_MENU, { panelId = self:GetPanelId(), isGroup = true, id = self.view.inputBindingGroupMonoTarget.groupId, hintPlaceholder = self.view.controllerHintPlaceholder, rectTransform = self.view.main.transform, })
...
FilterList._OnDisable = HL.Override() << function(self)
    ...
    Notify(MessageConst.CLOSE_CONTROLLER_SMALL_MENU, self:GetPanelId())
end
