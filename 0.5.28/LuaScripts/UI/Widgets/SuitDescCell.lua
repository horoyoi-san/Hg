...
SuitDescCell.InitSuitDescCellByEquipTemplateId = HL.Method(HL.String) << function(self, equipTemplateId)
    ...
    if hasValue then
        ...
        self.view.equipmentLogo.sprite = self.loader:LoadSprite(UIUtils.getSpritePath(UIConst.UI_SPRITE_EQUIPMENT_LOGO_BIG, suitData.suitIcon))
    else
        self.view.equipmentLogo.gameObject:SetActive(false)
    end
    LayoutRebuilder.ForceRebuildLayoutImmediate(self.view.transform)
end
SuitDescCell.InitSuitDescCell = HL.Method(HL.String, HL.Number, HL.Function, HL.Opt(HL.Boolean)) << function(self, suitId, suitCount, callback, onlyEnable)
    self:_FirstTimeInit()
    local hasValue, EquipSuitClientDataList = Tables.equipSuitTable:TryGetValue(suitId)
    self.view.gameObject:SetActive(hasValue)
    if not hasValue then
    ...
    self.m_descCellCache:Refresh(count, function(cell, index)
        ...
        cell.reachIcon.sprite = self:LoadSprite(UIConst.UI_SPRITE_ITEM_TIPS, reachIconName)
        cell.descText.text = descInfo.desc
    ...
    self.view.button.onClick:AddListener(function()
        callback({ cell = self, suitId = suitId, transform = self.view.button.transform, })
    end)
    LayoutRebuilder.ForceRebuildLayoutImmediate(self.view.transform)
end
