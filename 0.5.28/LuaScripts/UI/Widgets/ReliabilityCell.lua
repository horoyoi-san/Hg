...
ReliabilityCell._RefreshFriendship = HL.Method() << function(self)
    ...
    friendshipRoot.circleCur.fillAmount = isMax and 1.0 or realPercent / 100
...
ReliabilityCell._RefreshText = HL.Method() << function(self)
    ...
    local percent, _ = self:_GetCurFriendshipPercent()
    self.view.textRoot.textCur.text = tostring(percent) .. "%"
...
ReliabilityCell._StartSuccessCor = HL.Method(HL.Number, HL.Table) << function(self, deltaFav, selectedItems)
    ...
    for itemId, num in pairs(selectedItems) do
        if num > 0 then
            table.insert(items, { itemId = itemId, num = num, })
        end
    end
