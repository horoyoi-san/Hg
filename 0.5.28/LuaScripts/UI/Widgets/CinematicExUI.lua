...
CinematicExUI._OnFirstTimeInit = HL.Override() << function(self)
    ...
    self.view.touch.onClick:AddListener(function()
        if self.m_exNodeExistTime < 0 then
            ...
            self:RefreshExistTime(UIConst.CINEMATIC_TWEEN_TIME)
        end
    end)
    self:RegisterMessage(MessageConst.ON_INPUT_DEVICE_TYPE_CHANGED, function(arg)
        self:_OnInputDeviceChanged(arg)
    end)
end
...
CinematicExUI._OnInputDeviceChanged = HL.Method(HL.Any) << function(self, arg)
