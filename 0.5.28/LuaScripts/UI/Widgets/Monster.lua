...
Monster.InitMonster = HL.Method(HL.Opt(HL.Any, HL.Any, HL.String, HL.Boolean)) << function(self, monsterTemplateId, onClick, clickableEvenEmpty)
    self:_FirstTimeInit()
    self:_ResetOnInit()
    ...
    if self.view.config.SHOW_HOVER_TIP and not isEmpty then
        self.view.button.onHoverChange:AddListener(function(isHover)
            ...
            if isHover and not self.m_isSelected then
                Notify(MessageConst.SHOW_COMMON_HOVER_TIP, { mainText = data.name, subText = typeInfo.name, delay = self.view.config.HOVER_TIP_DELAY, })
...
Monster.SetSelected = HL.Method(HL.Opt(HL.Boolean)) << function(self, isSelected)
...
Monster._UpdateIcon = HL.Method(HL.Opt(HL.Any)) << function(self, data)
    ...
    self.view.icon.sprite = UIUtils.loadSprite(self.loader, loadPath, data.templateId)
end
