...
FacCachePipe._GetPipeArrowDefaultAnimName = HL.Method(HL.Number).Return(HL.String) << function(self, index)
    if self.view.config.USE_OVERRIDE_ANIMATION then
        local overrideCfg = string.format(ARROW_ANIMATION_OVERRIDE_NAME_DEFAULT, index)
        local overrideName = self.view.config[overrideCfg]
        if not string.isEmpty(overrideName) then
            return overrideName
        end
    end
    return string.format(ARROW_ANIMATION_NAME_DEFAULT, index)
end
FacCachePipe._GetPipeArrowLoopAnimName = HL.Method(HL.Number).Return(HL.String) << function(self, index)
    if self.view.config.USE_OVERRIDE_ANIMATION then
        local overrideCfg = string.format(ARROW_ANIMATION_OVERRIDE_NAME_LOOP, index)
        local overrideName = self.view.config[overrideCfg]
        if not string.isEmpty(overrideName) then
            return overrideName
        end
    end
    return string.format(ARROW_ANIMATION_NAME_LOOP, index)
end
FacCachePipe._GetPipeItemChangedAnimName = HL.Method(HL.Number).Return(HL.String) << function(self, index)
    if self.view.config.USE_OVERRIDE_ANIMATION then
        local overrideCfg = string.format(ITEM_CHANGED_ANIMATION_OVERRIDE_NAME, index)
        local overrideName = self.view.config[overrideCfg]
        if not string.isEmpty(overrideName) then
            return overrideName
        end
    end
    return string.format(ITEM_CHANGED_ANIMATION_NAME, index)
end
...
FacCachePipe._InitPipeList = HL.Method() << function(self)
    if self.m_useSinglePipe then
        ...
        self.m_inPipeList = { self.view.pipeCell1, self.view.pipeCell2, }
        self.m_outPipeList = { self.view.pipeCell3, self.view.pipeCell4, }
    end
    self.m_outPipeList = { self.m_useSinglePipe and self.view.singleOutCell or self.view.pipeCell3, self.m_useSinglePipe and nil or self.view.pipeCell4, }
    self.m_inBindingPipeDataMap = {}
    self.m_outBindingPipeDataMap = {}
    local pipeList = lume.concat(self.m_inPipeList, self.m_outPipeList)
    for index, pipeCell in ipairs(pipeList) do
        pipeCell.gameObject:SetActive(false)
        local animIndex = self.m_useSinglePipe and index + SINGLE_ANIM_INDEX_OFFSET or index
        pipeCell.defaultAnimationName = self:_GetPipeArrowDefaultAnimName(animIndex)
        pipeCell.loopAnimationName = self:_GetPipeArrowLoopAnimName(animIndex)
        pipeCell.itemChangedAnimationName = self:_GetPipeItemChangedAnimName(animIndex)
    end
...
FacCachePipe._InitPipeCell = HL.Method(HL.Any, HL.Table, HL.Table) << function(self, cell, info, bindingMap)
    ...
    if info.isBinding then
        bindingMap[info.touchNodeId] = { pipeCell = cell, touchCompId = info.touchCompId, }
    end
end
...
FacCachePipe._RefreshPipeCellState = HL.Method(HL.Any, HL.Table) << function(self, cell, info)
    ...
    local decoAnimationName = info.isBlock and cell.defaultAnimationName or cell.loopAnimationName
...
FacCachePipe._RefreshPipeCellConveyorAnimation = HL.Method(HL.Boolean, HL.Number, HL.Number, HL.String) << function(self, isIn, nodeId, compId, itemId)
    ...
    cell.bindingNode.itemAnimation:PlayWithTween(cell.itemChangedAnimationName)
