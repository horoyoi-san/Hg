...
ControllerHintPlaceholder.InitControllerHintPlaceholder = HL.Method(HL.Table, HL.Opt(HL.Table, HL.Function)) << function(self, groupIds, optionalActionIds, customGetKeyHintInfos)
    self:_InitPlaceholder({ groupIds = groupIds, optionalActionIds = optionalActionIds, customGetKeyHintInfos = customGetKeyHintInfos, })
end
...
ControllerHintPlaceholder.GetArgs = HL.Override().Return(HL.Table) << function(self)
    return { panelId = self.m_panelId, placeHolderObject = self.gameObject, groupIds = self.m_groupIds, optionalActionIds = self.m_optionalActionIds, isMain = self.config.IS_MAIN, offset = self.config.PANEL_ORDER_OFFSET, transform = self.view.transform, posType = self.view.config.POS_TYPE, useBG = self.view.config.USE_BG, customGetKeyHintInfos = self.m_customGetKeyHintInfos }
end
ControllerHintPlaceholder.GetHideArgs = HL.Override().Return(HL.Any) << function(self)
    return { panelId = self.m_panelId, placeHolderObject = self.gameObject, }
end
