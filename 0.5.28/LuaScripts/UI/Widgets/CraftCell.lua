...
CraftCell.InitCraftCell = HL.Method(HL.Any, HL.Opt(HL.Table)) << function(self, craftInfo, itemTipsPosInfo)
    self:_FirstTimeInit()
...
CraftCell._RefreshCraftCell = HL.Method() << function(self)
    ...
    if craftInfo.time then
        ...
        self.view.time.text = string.format(Language["LUA_CRAFT_CELL_STANDARD_TIME"], FactoryUtils.getCraftTimeStr(craftInfo.time))
    else
    ...
    local showPin = self.config.SHOW_PIN_BTN and not string.isEmpty(craftInfo.craftId) and Tables.factoryMachineCraftTable:ContainsKey(craftInfo.craftId)
