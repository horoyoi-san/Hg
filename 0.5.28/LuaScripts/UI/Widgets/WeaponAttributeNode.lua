...
WeaponAttributeNode.InitWeaponAttributeNodeByTemplateId = HL.Method(HL.String, HL.Opt(HL.Boolean)) << function(self, weaponTemplateId, isMaxLevel)
    ...
    self.m_subAttributeCellCache:Refresh(0, nil)
    self.m_extraAttributeCellCache:Refresh(0, nil)
    local mainAttributeList = isMaxLevel and CharInfoUtils.getWeaponShowAttributesByTemplateIdWithMaxLevel(weaponTemplateId) or CharInfoUtils.getWeaponShowAttributesByTemplateIdWithBasicLevel(weaponTemplateId)
    self.m_mainAttributeCellCache:Refresh(#mainAttributeList, function(cell, index)
        local attributeInfo = mainAttributeList[index]
        self:_RefreshAttributeCell(cell, attributeInfo)
    end)
    ...
    if extraArg and extraArg.showAttrTransition == true then
        self.m_effectCor = self:_StartCoroutine(function()
            self.m_mainAttributeCellCache:Refresh(#showTargetAttrs, function(cell, index)
                ...
                self:_RefreshUpgradeAttributeCell(cell, baseAttributeInfo, attributeInfo, true)
...
WeaponAttributeNode._Init = HL.Method(HL.Table, HL.Table, HL.Table, HL.Opt(HL.String, HL.Number)) << function(self, fcAttrShowList, extraAttrShowList, defShowInfo, charTemplateId, equipInstId)
    ...
    local isShowEnhance = false
    self.m_subAttributeCellCache:Refresh(#fcAttrShowList, function(cell, index)
        local attrInfo = fcAttrShowList[index]
        cell.equipEnhanceLevelNode.gameObject:SetActive(isShowEnhance)
        cell.equipEnhanceLevelNode:InitEquipEnhanceLevelNode({ equipInstId = equipInstId, attrIndex = attrInfo.enhancedAttrIndex, })
        cell.transform:SetSiblingIndex(siblingIndex)
        siblingIndex = siblingIndex + 1
        self:_RefreshAttributeCell(cell, attrInfo, true)
    end)
...
WeaponAttributeNode._RefreshAttributeCell = HL.Method(HL.Table, HL.Table, HL.Opt(HL.Boolean)) << function(self, cell, attributeInfo, showPlus)
    ...
    if cell.circleNode then
        cell.circleNode.gameObject:SetActive(not hasAttrIcon)
        cell.attributeIcon.gameObject:SetActive(hasAttrIcon)
    end
    if hasAttrIcon then
