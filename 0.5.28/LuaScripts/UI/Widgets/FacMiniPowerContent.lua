...
local BUILDING_ICON_MAP = { [GEnums.FacBuildingType.TravelPole] = "building_icon_travel_pole", [GEnums.FacBuildingType.Battle] = "building_icon_battle", }
    ...
    if buildingMode then
        ...
        if settlementId ~= nil then
            if settlementId == "" then
                bandwidth = sceneMsg.bandwidth
            else
                bandwidth = sceneMsg:GetSettlementBandwidth(settlementId)
            end
        end
    end
    if not inFacMain then
        local buildingCoundNodeActive = false
        local max = 0
        local cur = 0
        local delta = 0
        if data.type == GEnums.FacBuildingType.TravelPole then
        ...
        elseif data.type == GEnums.FacBuildingType.Battle then
            buildingCoundNodeActive = true
            if bandwidth then
        ...
        if buildingCoundNodeActive then
            local buildingIcon = self:LoadSprite(UIConst.UI_SPRITE_MINI_POWER, BUILDING_ICON_MAP[data.type])
            if buildingIcon ~= nil then
                self.view.buildingIcon.sprite = buildingIcon
            end
        end
