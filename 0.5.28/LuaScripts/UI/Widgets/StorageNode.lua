...
StorageNode.InitStorageNode = HL.Method(HL.Number, HL.Opt(HL.Number, HL.Boolean, HL.Boolean)) << function(self, count, needCount, ignoreInSafeZone, itemBagOnly)
    ...
    self.view.storageText.text = UIUtils.setCountColor(prefix, count < needCount)
    self.view.storageCount.text = UIUtils.setCountColor(UIUtils.getNumString(count), count < needCount)
end
