...
ItemSelectList._OnFirstTimeInit = HL.Override() << function(self)
    ...
    self.m_sortOptions = { { name = Language.LUA_FAC_CRAFT_SORT_1, keys = { "notSelected", "count", "sortId1", "sortId2", "id", "instId" }, }, { name = Language.LUA_FAC_CRAFT_SORT_2, keys = { "notSelected", "count", "rarity", "sortId1", "sortId2", "id", "instId" }, }, }
    self:RegisterMessage(MessageConst.ON_ITEM_LOCKED_STATE_CHANGED, function(arg)
        self:_OnItemLockedStateChanged(arg)
    end)
end
ItemSelectList.InitItemSelectList = HL.Method(HL.Table, HL.Table, HL.Function, HL.Opt(HL.String, HL.Function)) << function(self, itemIds, selectedItems, onClickItem, title, onUnlockItem)
    self:_FirstTimeInit()
    ...
    for i = 1, #itemIds do
        ...
        if typeData.storageSpace == GEnums.ItemStorageSpace.ValuableDepot then
            ...
            table.insert(items, { id = itemId, count = itemCount, sortId1 = itemData.sortId1, sortId2 = itemData.sortId2, rarity = itemData.rarity, notSelected = notSelected, })
        end
    end
    self.items = items
