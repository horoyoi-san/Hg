...
local IGNORE_TYPE_ICON_COLOR_LIST = { [MarkInfoType.MainMission:GetHashCode()] = true, [MarkInfoType.SideMission:GetHashCode()] = true, [MarkInfoType.FactoryMission:GetHashCode()] = true, [MarkInfoType.WorldMission:GetHashCode()] = true, [MarkInfoType.BlocMission:GetHashCode()] = true, }
...
FilterMarkCategoryCell._InitCategoryTypeDataList = HL.Method() << function(self)
    ...
    for filterType, filterData in pairs(Tables.mapMarkTypeTable) do
        if filterData.category:GetHashCode() == self.m_category then
            table.insert(self.m_typeDataList, { type = filterType, name = filterData.name, icon = filterData.icon, sortId = filterData.sortId, })
        end
    end
...
FilterMarkCategoryCell._InitCategoryTypeCells = HL.Method() << function(self)
    ...
    self.m_typeCells:Refresh(#self.m_typeDataList, function(cell, index)
        ...
        local iconSprite = self:LoadSprite(UIConst.UI_SPRITE_MAP_ICON, data.icon)
        if iconSprite ~= nil then
            cell.typeIcon.sprite = iconSprite
        end
    ...
    for index = 1, count do
        ...
        if cell ~= nil and data ~= nil then
            ...
            if IGNORE_TYPE_ICON_COLOR_LIST[data.type] then
                ...
                cell.typeIcon.color = filterActive and self.view.config.NOT_SELECT_TYPE_CELL_ICON_COLOR or self.view.config.SELECT_TYPE_CELL_ICON_COLOR
            end
        end
    end
end
