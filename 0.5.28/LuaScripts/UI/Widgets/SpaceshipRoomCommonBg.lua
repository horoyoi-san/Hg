...
SpaceshipRoomCommonBg.InitSpaceshipRoomCommonBg = HL.Method(HL.String, HL.Function, HL.Function) << function(self, roomId, returnFunc, closeFunc)
    self:_FirstTimeInit()
    ...
    self.view.infoNode.gameObject:SetActiveIfNecessary(roomInsData.roomType == GEnums.SpaceshipRoomType.ManufacturingStation)
    if roomInsData.roomType == GEnums.SpaceshipRoomType.ManufacturingStation then
        self.view.ingTxt.text = Language.LUA_SPACESHIP_MANUFACTURING_STATION_IS_PRODUCING
    elseif roomInsData.roomType == GEnums.SpaceshipRoomType.GrowCabin then
...
SpaceshipRoomCommonBg.RefreshTopInfoNode = HL.Method(HL.String) << function(self, efficacyTxt)
