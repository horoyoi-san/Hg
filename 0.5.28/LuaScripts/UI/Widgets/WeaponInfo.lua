...
WeaponInfo._OnFirstTimeInit = HL.Override() << function(self)
    ...
    self.view.buttonNode.buttonEquip.onClick:AddListener(function()
    ...
    self.view.charWeaponBasicNode.btnUpgrade.onClick:AddListener(function()
        ...
        local isTrailCard = CharInfoUtils.checkIsCardInTrail(exhibitInfo.weaponInst.equippedCharServerId)
        if isTrailCard then
        ...
        PhaseManager:SendMessageToOpenedPhase(PhaseId.CharInfo, MessageConst.P_CHAR_INFO_BLEND_EXIT, {
            finishCallback = function()
                CharInfoUtils.openWeaponInfoBestWay({ weaponTemplateId = exhibitInfo.weaponInst.templateId, weaponInstId = exhibitInfo.weaponInst.instId, pageType = UIConst.WEAPON_EXHIBIT_PAGE_TYPE.UPGRADE, isFocusJump = true, })
            end
        })
    end)
    ...
    self.view.charWeaponBasicNode.btnPotential.onClick:AddListener(function()
        ...
        PhaseManager:SendMessageToOpenedPhase(PhaseId.CharInfo, MessageConst.P_CHAR_INFO_BLEND_EXIT, {
            finishCallback = function()
                CharInfoUtils.openWeaponInfoBestWay({ weaponTemplateId = exhibitInfo.weaponInst.templateId, weaponInstId = exhibitInfo.weaponInst.instId, pageType = UIConst.WEAPON_EXHIBIT_PAGE_TYPE.POTENTIAL, isFocusJump = true, })
            end
        })
    end)
end
WeaponInfo._InitWeaponInfo = HL.Method(HL.Table, HL.Opt(HL.Number)) << function(self, weaponInfo, tryGemInstId)
    self:_FirstTimeInit()
    ...
    self:_RefreshSkill(weaponInstId, tryGemInstId)
...
WeaponInfo._RefreshNameNode = HL.Method(HL.Table) << function(self, weaponExhibitInfo)
    ...
    self.view.nameNode.typeIcon.sprite = self:LoadSprite(UIConst.UI_SPRITE_WEAPON_EXHIBIT, UIConst.WEAPON_EXHIBIT_WEAPON_TYPE_ICON_PREFIX .. weaponTypeInt)
...
WeaponInfo._RefreshBasicNode = HL.Method(HL.Opt(HL.Table)) << function(self, arg)
    ...
    weaponBasicNode.btnPreview.onClick:AddListener(function()
        UIManager:Open(PanelId.WeaponExhibitPreview, { weaponTemplateId = weaponExhibitInfo.weaponInst.templateId, weaponInstId = weaponExhibitInfo.weaponInst.instId, })
    end)
    ...
    charWeaponBasicNode.weaponGemNode:InitWeaponGemNode(attachedGemInstId, canEditGem == true, function()
        ...
        PhaseManager:SendMessageToOpenedPhase(PhaseId.CharInfo, MessageConst.P_CHAR_INFO_BLEND_EXIT, {
            finishCallback = function()
                CharInfoUtils.openWeaponInfoBestWay({ weaponTemplateId = weaponExhibitInfo.weaponInst.templateId, weaponInstId = weaponExhibitInfo.weaponInst.instId, pageType = UIConst.WEAPON_EXHIBIT_PAGE_TYPE.GEM, isFocusJump = true, })
            end
        })
    end)
...
WeaponInfo._RefreshGemCompareNode = HL.Method(HL.Table, HL.Opt(HL.Number)) << function(self, weaponExhibitInfo, tryGemInstId)
    ...
    self.view.gemCompareNode.itemBlack:InitItem({ id = gemTemplateId, instId = tryGemInstId, }, true)
end
...
WeaponInfo._RefreshSkill = HL.Method(HL.Number, HL.Opt(HL.Number)) << function(self, weaponInstId, tryGemInstId)
    local weaponInst = CharInfoUtils.getWeaponByInstId(weaponInstId)
    self.view.weaponSkillNode:InitWeaponSkillNode(weaponInstId, { tryRefineLv = weaponInst.refineLv, tryBreakthroughLv = weaponInst.breakthroughLv, tryGemInstId = tryGemInstId })
end
WeaponInfo._RefreshButtonState = HL.Method(HL.Table) << function(self, weaponInfo)
    self.view.buttonNode.buttonEquip.gameObject:SetActive(weaponInfo.showEquip == true)
