...
PRTSMainCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.ON_UNLOCK_PRTS] = '_OnUnlockStoryColl', [MessageConst.ON_INVESTIGATE_FINISHED] = '_OnInvestigateFinished', }
...
PRTSMainCtrl._UpdateData = HL.Method() << function(self)
    self.m_info = { textCount = GameInstance.player.prts:GetUnlockCountByPageType(self.m_textPageType), documentCount = GameInstance.player.prts:GetUnlockCountByPageType(self.m_documentPageType), multimediaCount = GameInstance.player.prts:GetUnlockCountByPageType(self.m_multimediaPageType), investFinishedCount = 0, investOnGoingCount = 0, }
