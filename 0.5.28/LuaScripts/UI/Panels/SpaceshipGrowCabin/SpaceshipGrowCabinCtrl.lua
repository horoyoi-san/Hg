...
local PanelType = { Overview = 1, Sow = 2, Breed = 3, }
...
SpaceshipGrowCabinCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.SPACESHIP_ON_SYNC_ROOM_STATION] = "OnSpaceshipRoomStationSync", [MessageConst.ON_SPACESHIP_GROW_CABIN_MODIFY] = 'OnSpaceshipGrowCabinModify', [MessageConst.ON_SPACESHIP_GROW_CABIN_SOW] = 'OnSpaceshipGrowCabinSow', [MessageConst.ON_SPACESHIP_GROW_CABIN_CANCEL] = 'OnSpaceshipGrowCabinCancel', [MessageConst.ON_SPACESHIP_GROW_CABIN_HARVEST] = "OnSpaceshipGrowCabinHarvest", [MessageConst.ON_SPACESHIP_GROW_CABIN_BREED] = "OnSpaceshipGrowCabinBreed", }
...
SpaceshipGrowCabinCtrl.OnCreate = HL.Override(HL.Any) << function(self, arg)
    ...
    self.m_autoSelectNextSowBox = ClientDataManagerInst:GetBool(AUTO_SELECT_NEXT_SOW_BOX_SWITCH_KEY, false, true, CLIENT_DATA_MANAGER_CATEGORY)
...
SpaceshipGrowCabinCtrl.OnClose = HL.Override() << function(self)
    ...
    ClientDataManagerInst:SetBool(AUTO_SELECT_NEXT_SOW_BOX_SWITCH_KEY, self.m_autoSelectNextSowBox, false, CLIENT_DATA_MANAGER_CATEGORY, true)
...
SpaceshipGrowCabinCtrl._RefreshOverviewPanel = HL.Method(HL.Opt(HL.Any)) << function(self, ids)
...
SpaceshipGrowCabinCtrl._RefreshSowPanel = HL.Method(HL.Opt(HL.Boolean)) << function(self, isInit)
    ...
    local spaceship = GameInstance.player.spaceship
    local formulaData = Tables.spaceshipGrowCabinFormulaTable[self.m_curSelectSowFormulaId]
    ...
    node.timeTxt.text = UIUtils.getLeftTimeToSecondFull(formulaData.totalProgress / produceRate)
    if isInit then
    ...
    if not selectSowFormulaUnlock then
        node.stateTxt.text = string.format(Language.LUA_SPACESHIP_ROOM_GROW_CABIN_BOX_UNLOCK_CONDITION_FORMAT, formulaData.level)
    end
end
SpaceshipGrowCabinCtrl._RefreshSowPanelTimeInfo = HL.Method() << function(self)
    ...
    if not succ or string.isEmpty(box.scdMsg.RecipeId) or box.scdMsg.IsReady then
        self.view.cultivationObject.timeTxt.text = UIUtils.getLeftTimeToSecondFull(0)
        return
    end
    local node = self.view.cultivationObject
    ...
    node.timeTxt.text = UIUtils.getLeftTimeToSecondFull((totalProgress - curProgress) / produceRate)
end
...
SpaceshipGrowCabinCtrl._RefreshBreedFormulaList = HL.Method(HL.Opt(HL.Table)) << function(self, args)
    ...
    formulaData.sortOptions = { { name = Language.LUA_FAC_CRAFT_SORT_1, keys = { "isUnlockSortId", "rarity", "sortId" } }, { name = Language.LUA_SPACESHIP_ROOM_GROW_CABIN_BREED_SORT_2, keys = { "materialCount", "rarity", "sortId" } }, { name = Language.LUA_SPACESHIP_ROOM_GROW_CABIN_RARITY_SORT, keys = { "rarity", "sortId" } }, }
    formulaData.onCellClick = function(info)
...
SpaceshipGrowCabinCtrl._ProcessBreedFormulaTabListData = HL.Method().Return(HL.Table) << function(self)
    ...
    for _, formulaData in pairs(Tables.spaceshipGrowCabinSeedFormulaTable) do
        ...
        local formulaDataUnit = { formulaId = formulaData.id, itemId = materialItemData.id, roomId = self.m_roomId, isUnlock = isUnlock, materialCount = Utils.getItemCount(formulaData.materialItemId), rarity = formulaData.rarity, sortId = formulaData.sortId, isUnlockSortId = isUnlock and 1 or 0, }
...
SpaceshipGrowCabinCtrl._RefreshBreedPanel = HL.Method() << function(self)
    ...
    local spaceship = GameInstance.player.spaceship
    local formulaData = Tables.SpaceshipGrowCabinSeedFormulaTable[self.m_curSelectBreedFormulaId]
    local materialItemData = Tables.itemTable[formulaData.materialItemId]
    ...
    if not selectBreedFormulaUnlock then
        node.stateTxt.text = string.format(Language.LUA_SPACESHIP_ROOM_GROW_CABIN_BREED_FORMULA_UNLOCK_CONDITION_FORMAT, formulaData.level)
...
SpaceshipGrowCabinCtrl._OnAllContinueSowBtnClick = HL.Method() << function(self)
    ...
    for itemId, item in pairs(itemMap) do
        table.insert(items, { id = itemId, needCount = item.count, count = item.ownCount, })
    end
...
SpaceshipGrowCabinCtrl._ShowOutcomePopup = HL.Method(HL.String, HL.Any) << function(self, title, csItems)
    local itemMap = {}
    for i = 0, csItems.Count - 1 do
        local item = csItems[i]
        if itemMap[item.Id] then
            local accCount = itemMap[item.Id]
            itemMap[item.Id] = accCount + item.Count
        else
    ...
    Notify(MessageConst.SHOW_SYSTEM_REWARDS, { title = title, items = items, })
end
...
SpaceshipGrowCabinCtrl.OnSpaceshipGrowCabinModify = HL.Method(HL.Any) << function(self, args)
    ...
    self:_RefreshBgBottomNode()
    self:_RefreshOverviewPanel(ids)
end
