...
local InfoAtFirstHighlightStyle = { Force = "Force", Weak = "Weak", FullScreenForce = "FullScreenForce", }
local uiCtrl = require_ex('UI/Panels/Base/UICtrl')
local PANEL_ID = PanelId.Guide
GuideCtrl = HL.Class('GuideCtrl', uiCtrl.UICtrl)
GuideCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.HIDE_GUIDE_STEP] = 'HideGuideStep', [MessageConst.ON_INTERRUPT_GUIDE] = 'OnInterruptGuide', [MessageConst.ON_UI_PANEL_START_OPEN] = 'OnPanelOpened', [MessageConst.ON_UI_PANEL_SHOW] = 'OnPanelOpened', [MessageConst.ON_UI_PANEL_CLOSED] = 'OnPanelClosed', [MessageConst.ON_UI_PANEL_HIDE] = 'OnPanelClosed', [MessageConst.ON_UI_PHASE_OPENED] = 'OnPhaseOpened', [MessageConst.ON_UI_PHASE_EXITED] = 'OnPhaseExited', }
...
GuideCtrl.OnCreate = HL.Override(HL.Any) << function(self, arg)
    ...
    UIUtils.bindInputPlayerAction("guide_click", function()
        if DeviceInfo.isPCorConsole and not InputManager.cursorVisible then
            self:_TryGotoNextStep(true)
        end
    end, self.view.fullScreenBtn.groupId)
    self.m_uiHighlightCells = UIUtils.genCellCache(self.view.uiHighlightCell)
    self.m_dialogNodeMap = { [TextStyle.BigLeftTop] = self.view.bigLeftTopDialogNode, [TextStyle.BigLeftBottom] = self.view.bigLeftBottomDialogNode, [TextStyle.TitleMiddleTop] = self.view.titleMiddleTopNode, [TextStyle.InfoAtFirstUIHighlight] = self.view.infoAtFirstUIHighlightDialogNode, }
    self.m_highlightDialogPadding = { top = self.view.config.UI_HIGHLIGHT_DIALOG_PADDING_TOP, bottom = self.view.config.UI_HIGHLIGHT_DIALOG_PADDING_BOTTOM, left = self.view.config.UI_HIGHLIGHT_DIALOG_PADDING_LEFT, right = self.view.config.UI_HIGHLIGHT_DIALOG_PADDING_RIGHT, }
    ...
    if self.m_curUIHighlightInfos then
        ...
        for _, v in ipairs(self.m_curUIHighlightInfos) do
            if IsNull(v.targetTrans) or v.targetTrans.gameObject.name ~= v.oriName then
                ...
                if NotNull(v.targetTrans) then
                    v.button = self:_GetClickTarget(v.targetTrans)
                end
            end
            if NotNull(v.targetTrans) then
                local rect = UIUtils.getUIRectOfRectTransform(v.targetTrans, v.is3DUI and CameraManager.mainCamera or self.uiCamera)
                self:_SyncUIHighlightTargetRect(v.cell, rect, v.needMask)
                if not isForce then
                    local isActive = v.targetTrans.gameObject.activeInHierarchy and (not v.button or v.button.groupEnabled)
                ...
                if v.cell.gameObject.activeSelf ~= false then
...
GuideCtrl.ShowGuideStep = HL.StaticMethod(HL.Table) << function(args)
    ...
    self:_RefreshDialogControllerHint(stepInfo)
    ...
    CS.Beyond.UI.UIActionKeyHint.stopCheckBindingEnabledForGuide = needBlockInput and not isMedia and not isHelper
    self:ChangeCurPanelBlockSetting(needBlockInput)
    self.view.fullScreenBtn.gameObject:SetActive(needFullScreenBtn)
...
GuideCtrl.HideGuideStep = HL.Method(HL.Opt(HL.Any)) << function(self, args)
    ...
    CS.Beyond.UI.UIActionKeyHint.stopCheckBindingEnabledForGuide = false
...
GuideCtrl.ShowGuideEmptyMask = HL.StaticMethod() << function()
    local self = UIManager:AutoOpen(PANEL_ID)
    self.view.emptyMask.gameObject:SetActiveIfNecessary(true)
    self.view.main.gameObject:SetActiveIfNecessary(false)
    CS.Beyond.UI.UIActionKeyHint.stopCheckBindingEnabledForGuide = true
...
GuideCtrl._TryGotoNextStep = HL.Method(HL.Boolean) << function(self, checkInterval)
    ...
    if checkInterval and Time.unscaledTime < self.m_stepStartTime + UIConst.GUIDE_STEP_MIN_INTERVAL then
        return
    end
    self:_ClearStep()
...
GuideCtrl._RefreshDialog = HL.Method(HL.Userdata) << function(self, textInfos)
    ...
    for _, info in pairs(textInfos) do
        ...
        local result = UIUtils.resolveTextStyle(Language[info.txtId])
        node.content.text = InputManager.ParseTextActionId(result)
...
GuideCtrl._RefreshDialogHintNode = HL.Method(HL.Boolean) << function(self, needFullScreenBtn)
    for _, dialogNode in pairs(self.m_dialogNodeMap) do
        dialogNode.hintNode.gameObject:SetActiveIfNecessary(needFullScreenBtn)
    end
end
GuideCtrl._SyncInfoAtFirstUIHighlight = HL.Method(HL.Table) << function(self, info)
    ...
    if IsNull(info.targetTrans) then
        logger.warn("引导高亮UI节点已无效", info.path)
        return
    end
    ...
    UIUtils.updateTipsPosition(node.rectTransform, info.targetTrans, self.view.rectTransform, self.uiCamera, UIConst.UI_TIPS_POS_TYPE.GuideTips, self.m_highlightDialogPadding, hasArrow and self.view.config.DIALOG_NODE_ARROW_OFFSET_X or 0)
    self:_SyncUIHighlightDialogArrowState(info.targetTrans)
    local isForce = self.m_curStepInfo.type ~= GuideStepType.Weak
    if not isForce then
        local isActive = info.targetTrans.gameObject.activeInHierarchy and (not info.button or info.button.groupEnabled)
...
GuideCtrl._SyncUIHighlightDialogArrowState = HL.Method(HL.Userdata) << function(self, highlightRectTransform)
    ...
    local middle = highlightScreenRect.center.y >= dialogScreenRect.center.y - middleRangeOffset and highlightScreenRect.center.y <= dialogScreenRect.center.y + middleRangeOffset
    local arrowX, arrowY
    if middle then
        arrowY = 0
    else
...
GuideCtrl._RefreshHelper = HL.Method().Return(HL.Boolean) << function(self)
    ...
    helperNode.content.text = InputManager.ParseTextActionId(UIUtils.resolveTextStyle(Language[helperInfo.contentTextId]))
    local useCustomImage = not string.isEmpty(helperInfo.helperImagePath)
    if useCustomImage then
        helperNode.customHelperImage.sprite = self:LoadSprite(UIConst.UI_SPRITE_GUIDE, helperInfo.helperImagePath)
    end
...
GuideCtrl._RefreshHelperTrackPointDistance = HL.Method() << function(self)
    ...
    local distance = GameInstance.player.commonTrackingPoint:GetDistanceByTrackingPointKey(self.m_curTrackPointKey)
    if distance > 0 then
...
GuideCtrl._RefreshHelperVisibleStateByPanelOrPhaseState = HL.Method(HL.String, HL.Boolean, HL.Boolean) << function(self, name, isOpened, isPanel)
    ...
    local invisibleList = isPanel and Tables.globalConst.guideHelperInvisiblePanelList or Tables.globalConst.guideHelperInvisiblePhaseList
...
GuideCtrl._HighlightUITarget = HL.Method(HL.Table, CS.Beyond.Gameplay.GuideUIHighlightInfo, HL.Boolean) << function(self, cell, info, needMask)
    ...
    if not trans then
        logger.error(ELogChannel.Guide, "引导的UI路径不对", path)
    ...
    self:_SyncUIHighlightTargetRect(cell, targetRect, needMask)
    local uiInfo = { cell = cell, targetTrans = trans, oriName = trans.gameObject.name, is3DUI = is3DUI, path = info.path, isCircle = isCircle, needMask = needMask, }
    ...
    if self.m_isCurForceStep then
        if info.hintType == HintType.Click then
            if button or toggle or dropdown or keyHint then
                ...
                cell.button.pressStartTriggerClick = info.pressStartTriggerClick
                cell.button.onClick = function()
                    self:_OnClickFakeButton()
                end
                cell.button:CopyTargetBinding()
            else
                logger.error(ELogChannel.Guide, "引导：该路径下没有可点击对象", info.path)
            end
...
GuideCtrl._GetClickTarget = HL.Method(Transform).Return(HL.Opt(HL.Any, HL.Any, HL.Any, HL.Any)) << function(self, trans)
    local targets = {}
    local targetTypes = { typeof(CS.Beyond.UI.UIButton), typeof(CS.Beyond.UI.UIToggle), typeof(CS.Beyond.UI.UIDropdown), typeof(CS.Beyond.UI.UIActionKeyHint), }
...
GuideCtrl._SyncUIHighlightTargetRect = HL.Method(HL.Table, Unity.Rect, HL.Boolean) << function(self, cell, targetRect, needMask)
    ...
    if otherCell and otherCell ~= cell then
        local selfRect = { xMin = rectTrans.anchoredPosition.x - rectTrans.sizeDelta.x / 2, xMax = rectTrans.anchoredPosition.x + rectTrans.sizeDelta.x / 2, yMin = rectTrans.anchoredPosition.y - rectTrans.sizeDelta.y / 2, yMax = rectTrans.anchoredPosition.y + rectTrans.sizeDelta.y / 2, }
        local otherRect = { xMin = otherCell.rectTransform.anchoredPosition.x - otherCell.rectTransform.sizeDelta.x / 2, xMax = otherCell.rectTransform.anchoredPosition.x + otherCell.rectTransform.sizeDelta.x / 2, yMin = otherCell.rectTransform.anchoredPosition.y - otherCell.rectTransform.sizeDelta.y / 2, yMax = otherCell.rectTransform.anchoredPosition.y + otherCell.rectTransform.sizeDelta.y / 2, }
        ...
        elseif selfRect.xMin > otherRect.xMax then
            ...
            logger.error(ELogChannel.Guide, "对象重合")
        end
    end
end
...
GuideCtrl._AdjustHorizontalTargetMasks = HL.Method(HL.Table, HL.Table, HL.Number) << function(self, rightCell, leftCell, uiWidth)
    local width = UIManager.uiCanvasRect.rect.size.x
...
GuideCtrl._InitDialogControllerHint = HL.Method() << function(self)
    ...
    self.m_controllerHintTextList = { self.view.bigLeftBottomDialogNode.controllerHintText, self.view.bigLeftTopDialogNode.controllerHintText, self.view.titleMiddleTopNode.controllerHintText, self.view.infoAtFirstUIHighlightDialogNode.controllerHintText, }
    self.m_controllerHintNodeList = { self.view.bigLeftBottomDialogNode.controllerHintNode, self.view.bigLeftTopDialogNode.controllerHintNode, self.view.titleMiddleTopNode.controllerHintNode, self.view.infoAtFirstUIHighlightDialogNode.controllerHintNode, }
    for _, hintText in ipairs(self.m_controllerHintTextList) do
        if hintText ~= nil then
            local result = UIUtils.resolveTextStyle(Language["ui_common_next_step"])
            hintText.text = InputManager.ParseTextActionId(result)
        end
    end
...
GuideCtrl._RefreshDialogControllerHint = HL.Method(HL.Userdata) << function(self, stepInfo)
    self:_RefreshDialogControllerHintNodeList(false)
    if not DeviceInfo.usingController then
        return
    end
    if stepInfo == nil or stepInfo.type == GuideStepType.Weak then
        return
    end
