...
SketchCtrl.ShowSketch = HL.Method(HL.Any) << function(self, sketchData)
    ...
    if leftActive then
        self.view.leftImage.sprite = self:LoadSprite(UIConst.UI_SPRITE_SKETCH, sketchData.imageLeft)
        self.view.leftImage:SetNativeSize()
    end
    if rightActive then
        self.view.rightImage.sprite = self:LoadSprite(UIConst.UI_SPRITE_SKETCH, sketchData.imageRight)
