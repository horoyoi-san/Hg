...
SettlementDefenseMainMapCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.ON_TOWER_DEFENSE_DEFENDING_ENEMY_KILLED] = '_RefreshEnemyCount', }
SettlementDefenseMainMapCtrl.OnCreate = HL.Override(HL.Any) << function(self, arg)
    ...
    self.m_enemyAbilityCells = UIUtils.genCellCache(self.view.enemyTipsNode.abilityCell)
...
SettlementDefenseMainMapCtrl._RefreshEnemyCount = HL.Method() << function(self)
    self.view.enemyCount.text = string.format(ENEMY_COUNT_FORMAT, self.m_towerDefenseGame.killedEnemyCount, self.m_towerDefenseGame.totalEnemyCount)
end
...
SettlementDefenseMainMapCtrl._ShowAndRefreshEnemyTips = HL.Method(HL.Number) << function(self, index)
    ...
    local enemySprite = self:LoadSprite(UIConst.UI_SPRITE_MONSTER_ICON, enemyInfo.templateId)
    if enemySprite ~= nil then
        enemyTipsNode.enemyImg.sprite = enemySprite
    end
    enemyTipsNode.descTxt.text = UIUtils.resolveTextStyle(enemyInfo.desc)
    ...
    if #abilityList > 0 then
        self.m_enemyAbilityCells:Refresh(#abilityList, function(cell, abilityIndex)
            cell.abilityDescTxt.text = UIUtils.resolveTextStyle(abilityList[abilityIndex].description)
        end)
        enemyTipsNode.abilityNode.gameObject:SetActive(true)
    else
        enemyTipsNode.abilityNode.gameObject:SetActive(false)
    end
