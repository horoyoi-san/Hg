...
PRTSInvestigateGalleryCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.ON_UNLOCK_PRTS] = '_OnUnlockStoryColl', [MessageConst.ON_INVESTIGATE_FINISHED] = '_OnInvestigateFinished', [MessageConst.ON_READ_PRTS_NOTE_BATCH] = '_OnNoteStateChange', }
local InvestStateEnum = { IsLock = 0, IsFinished = 1, IsOngoing = 2, IsCanReward = 3, }
...
PRTSInvestigateGalleryCtrl._UpdateData = HL.Method() << function(self)
    ...
    for id, investCfg in pairs(Tables.prtsInvestigate) do
        ...
        local info = { id = id, title = investCfg.name, type = investCfg.type, iconPath = iconPath, index = investCfg.index, curCount = curCount, targetCount = targetCount, state = infoState, stateOrder = stateOrder, }
...
PRTSInvestigateGalleryCtrl._OnRefreshInvestCell = HL.Method(HL.Table, HL.Number) << function(self, cell, luaIndex)
    ...
    cell.iconImg.sprite = self:LoadSprite(UIConst.UI_SPRITE_PRTS, realIconPath)
