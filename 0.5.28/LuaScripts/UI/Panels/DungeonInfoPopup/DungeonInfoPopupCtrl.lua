...
DungeonInfoPopupCtrl.TryToShow = HL.StaticMethod(HL.Any) << function(args)
    ...
    if not DungeonUtils.isDungeonHasFeatureInfo(subGameId) or GameInstance.dungeonManager:IsDungeonPassed(subGameId) then
        return
    end
    LuaSystemManager.commonTaskTrackSystem:AddRequest("DungeonInfo", function()
        DungeonInfoPopupCtrl.AutoOpen(PANEL_ID, {
            dungeonId = subGameId,
            closeCb = function()
...
DungeonInfoPopupCtrl._Refresh = HL.Method() << function(self)
    ...
    if hasFeature then
        self.m_dungeonInfoCellCache:Refresh(#featureInfos, function(cell, index)
            cell.txt.text = UIUtils.resolveTextStyle(featureInfos[index])
        end)
    end
