...
local GRADE_TEXT = { Language.LUA_SSSHOP_UNLOCK_ONE, Language.LUA_SSSHOP_UNLOCK_TWO, Language.LUA_SSSHOP_UNLOCK_THREE, Language.LUA_SSSHOP_UNLOCK_FOUR, }
local GRADE_LOOP = 4
...
MapMarkDetailSSShopCtrl.OnCreate = HL.Override(HL.Any) << function(self, args)
    ...
    local commonArgs = { markInstId = markInstId, bigBtnActive = true, }
    ...
    local strLastSeeTimeStamp = ClientDataManagerInst:GetString(SSSHOP_DETAIL_CLIENT_DATA_MANAGER_LAST_SEEN_TIMESTAMP_KEY, false, "0")
...
MapMarkDetailSSShopCtrl._FillSingleShop = HL.Method(HL.Any, HL.String).Return(HL.Boolean, HL.Boolean) << function(self, singleShop, shopGroupId)
    ...
    singleShop.icon.sprite = self:LoadSprite(UIConst.UI_SPRITE_WALLET, moneyData.iconId)
