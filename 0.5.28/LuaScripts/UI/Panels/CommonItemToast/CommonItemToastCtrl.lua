...
CommonItemToastCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.SHOW_EXP_TOAST] = 'OnShowExpToast', [MessageConst.ON_ADVENTURE_EXP_CHANGE] = 'OnShowAdventureExpToast', [MessageConst.ON_ITEM_COUNT_CHANGED_FOR_TOAST] = 'OnItemCountChangedImm', [MessageConst.ON_WALLET_CHANGED] = 'OnWalletChangedImm', }
...
CommonItemToastCtrl.OnShowAdventureExpToast = HL.Method(HL.Any) << function(self, arg)
    ...
    local info = { itemId = Tables.globalConst.adventureExpItemId, count = curExp - preExp, }
...
CommonItemToastCtrl.OnItemCountChangedImm = HL.Method(HL.Any) << function(self, arg)
    ...
    for itemId, diffCount in pairs(itemId2DiffCount) do
        if diffCount > 0 then
            table.insert(toastDataList, { itemId = itemId, count = diffCount })
        end
    end
    self:_AddToastRequest(toastDataList)
end
local blackMap = { ["item_spaceship_tundra_gold"] = true, ["item_spaceship_jinlong_gold"] = true, }
CommonItemToastCtrl.OnWalletChangedImm = HL.Method(HL.Any) << function(self, arg)
    local itemId, curCount, diffCount = unpack(arg)
    if blackMap[itemId] then
        return
    end
    if diffCount == nil or diffCount <= 0 then
        return
    end
    self:_AddToastRequest({ { itemId = itemId, count = diffCount, } })
end
...
CommonItemToastCtrl._ShowNewToast = HL.Method() << function(self)
    ...
    self.m_toastTimeSchedule:Push({ showTime = Time.realtimeSinceStartup, scrollIndex = self.m_curListCount, })
...
CommonItemToastCtrl._UpdateItemCell = HL.Method(HL.Any, HL.Table) << function(self, toastCell, data)
    ...
    toastCell.num.text = UIUtils.getNumString(data.count)
...
CommonItemToastCtrl._AddToastRequest = HL.Method(HL.Any) << function(self, infoList)
    ...
    LuaSystemManager.mainHudToastSystem:AddRequest("GetItemToast", function()
