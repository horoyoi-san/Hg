...
LostAndFoundCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.ON_GET_LOST_AND_FOUND] = 'OnGetLostAndFound', [MessageConst.ON_ADD_LOST_AND_FOUND] = 'OnAddLostAndFound', [MessageConst.ON_LOST_AND_FOUND_NOTHING_GET] = 'OnLostAndFoundNothingGet' }
...
LostAndFoundCtrl._ProcessItem = HL.Method(HL.String, HL.Number, HL.Opt(HL.Number)).Return(HL.Table) << function(self, id, count, instId)
    local data = Tables.itemTable:GetValue(id)
    local info = { id = id, instId = instId or 0, isInst = instId ~= nil, count = count, maxStackCount = data.maxBackpackStackCount, data = data, showingType = data.showingType, rarity = data.rarity, sortId1 = data.sortId1, sortId2 = data.sortId2, }
    return info
end
