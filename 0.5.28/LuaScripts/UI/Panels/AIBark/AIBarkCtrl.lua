...
AIBarkCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.STOP_AI_BARK] = 'AudioStopSingleAIBark', [MessageConst.ON_SCENE_LOAD_START] = 'StopAllAIBark', [MessageConst.ALL_CHARACTER_DEAD] = 'StopAllAIBark', [MessageConst.ON_TELEPORT_SQUAD] = 'StopAllAIBark', [MessageConst.PLAY_CG] = 'StopAllAIBark', [MessageConst.ON_PLAY_CUTSCENE] = 'StopAllAIBark', [MessageConst.ON_DIALOG_START] = 'StopAllAIBark', [MessageConst.SHOW_RADIO] = 'StopAllAIBark', [MessageConst.HIDE_GUIDE_STEP] = 'StopAllAIBark', [MessageConst.START_GUIDE_GROUP] = 'StopAllAIBark', [MessageConst.ON_ULTIMATE_SKILL_START] = 'StopAllAIBark', [MessageConst.ON_ULTIMATE_SKILL_END] = 'StopAllAIBark', }
...
AIBarkCtrl.OnShow = HL.Override() << function(self)
    if AIBarkCtrl.s_showByExecute then
        GameInstance.world.aiBarkManager:UpdateFullScreenUICD()
    end
end
...
AIBarkCtrl.OnBarkStopped = HL.StaticMethod(HL.String) << function(barkId)
    GameInstance.world.aiBarkManager:UpdateBarkEndTime(barkId)
end
...
AIBarkCtrl._TryShowSingleBark = HL.Method(CS.Beyond.Gameplay.AIBarkManager.AIBarkRuntimeData) << function(self, aiBarkData)
    ...
    if res then
        ...
        if duration <= 0 then
            duration = UIUtils.getTextShowDuration(barkText, 1)
        end
        AIBarkCtrl.s_uniqueId = AIBarkCtrl.s_uniqueId + 1
        local bark = { barkId = barkId, cell = cell, voiceHandleId = voiceHandleId, barkTextId = barkTextId, barkText = barkText, isLastSentence = isLastSentence, needStopVoice = true, timerId = -1, uniqueId = AIBarkCtrl.s_uniqueId, startTime = Time.time, duration = duration, }
        cell.text.text = barkText
        local spriteName = UIConst.UI_AI_BARK_CHAR_HEAD_PREFIX .. "_" .. charTemplateId
        cell.chrIcon.sprite = self:LoadSprite(UIConst.UI_SPRITE_AI_BARK_CHAR_HEAD, spriteName)
