...
ShopItemPopUpCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.ON_BUY_ITEM_SUCC] = 'OnBuyItemSucc', [MessageConst.ON_SHOP_REFRESH] = 'PlayAnimationOutAndClose', [MessageConst.ON_SHOP_GOODS_CONDITION_REFRESH] = 'PlayAnimationOutAndClose', }
...
ShopItemPopUpCtrl._UpdateContent = HL.Method() << function(self)
    ...
    if ResourceManager.CheckExists(resPath) then
        ...
        if tryIcon2 then
            ...
            local icon = self:LoadSprite(UIConst.UI_SPRITE_WALLET, moneyItemData.iconId)
            self.view.moneyIcon.sprite = icon
            self.view.costMoneyIcon.sprite = icon
        end
    else
        local icon = self:LoadSprite(UIConst.UI_SPRITE_WALLET, moneyItemData.iconId)
        self.view.moneyIcon.sprite = icon
...
ShopItemPopUpCtrl.OnBuyItemSucc = HL.Method(HL.Any) << function(self, arg)
    ...
    if itemData.maxStackCount <= 1 and Utils.isItemInstType(itemId) then
        for i = 1, totalCount do
            local item = { id = itemId, count = 1, }
            table.insert(items, item)
        end
    else
        items = { { id = itemId, count = totalCount, } }
    end
    ...
    if itemData.type == GEnums.ItemType.Char then
        ...
        Notify(MessageConst.SHOW_SYSTEM_REWARDS, { title = Language.LUA_BUY_ITEM_SUCC_TITLE, icon = "icon_common_rewards", items = items, chars = rewardPack.chars, onComplete = onComplete })
    elseif itemData.type == GEnums.ItemType.Weapon then
        ...
        Notify(MessageConst.SHOW_SYSTEM_REWARDS, { title = Language.LUA_BUY_ITEM_SUCC_TITLE, icon = "icon_common_rewards", items = items, onComplete = onComplete, })
    else
        Notify(MessageConst.SHOW_SYSTEM_REWARDS, { title = Language.LUA_BUY_ITEM_SUCC_TITLE, icon = "icon_common_rewards", items = items, onComplete = onComplete, })
    end
end
