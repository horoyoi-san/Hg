...
FacBeltCtrl._UpdateBeltItems = HL.Method() << function(self)
    ...
    if wholeSuccess then
        ...
        for i = 0, wholeItemList.Count - 1 do
            ...
            if index == 0 then
                index = index + 1
                self.m_wholeBeltItems[index] = { id = currentItemId, count = 1, }
            else
...
FacBeltCtrl._RefreshSelfItem = HL.Method(HL.Boolean) << function(self, isEmpty)
    ...
    if success then
        self.view.selfItemIcon.sprite = self:LoadSprite(UIConst.UI_SPRITE_ITEM, itemData.iconId)
    ...
    self.view.selfItem:InitItem({ id = self.m_lastSelfValidItem, count = isEmpty and 0 or 1, }, true)
...
FacBeltCtrl._OnGetSelfItemButtonClicked = HL.Method() << function(self)
    self.m_buildingInfo.sender:Message_OpMoveItemConveyorToBag(Utils.getCurrentChapterId(), self.m_buildingInfo.conveyorComponentId, self.m_wholeBeltLength - self.m_index - 1, false)
end
FacBeltCtrl._OnGetWholeItemsButtonClicked = HL.Method() << function(self)
    self.m_buildingInfo.sender:Message_OpMoveItemConveyorToBag(Utils.getCurrentChapterId(), self.m_buildingInfo.conveyorComponentId, 1, true)
end
