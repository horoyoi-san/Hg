...
CharInfoTipsCtrl._ShowTacticalItemTips = HL.Method(HL.Table) << function(self, args)
    ...
    self.view.descSec.text = UIUtils.resolveTextStyle(UIUtils.getItemEquippedDesc(itemId))
    ...
    local itemCount = GameInstance.player.inventory:GetTacticalItemCount(Utils.getCurrentScope(), itemId, args.charInstId)
    ...
    if not args.isLocked then
        ...
        self.view.btnExchange.onClick:AddListener(function()
            CharInfoUtils.openCharInfoBestWay({ pageType = UIConst.CHAR_INFO_PAGE_TYPE.EQUIP, initCharInfo = { instId = args.charInstId, templateId = args.charTemplateId, isSingleChar = true, }, forceSkipIn = true, extraArg = { slotType = UIConst.CHAR_INFO_EQUIP_SLOT_MAP.TACTICAL } })
            self:Close()
        end)
    end
...
CharInfoTipsCtrl._UpdateLayoutAndPosition = HL.Method(HL.Table) << function(self, args)
    UIUtils.updateTipsPosition(self.view.content, args.targetTransform, self.view.rectTransform, self.uiCamera, args.tipPosType)
end
