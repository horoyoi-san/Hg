local GEM_RECAST_TAB_TYPE = { FORGE = 1, RECAST = 2, }
local GEM_RECAST_CTRL_TAB = { [GEM_RECAST_TAB_TYPE.FORGE] = { tabName = "tabForge", nodeName = "forgeNode", }, [GEM_RECAST_TAB_TYPE.RECAST] = { tabName = "tabRecast", nodeName = "recastNode", } }
local uiCtrl = require_ex('UI/Panels/Base/UICtrl')
local PANEL_ID = PanelId.GemRecast
GemRecastCtrl = HL.Class('GemRecastCtrl', uiCtrl.UICtrl)
GemRecastCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.ON_GEM_RECAST] = "OnGemRecast", [MessageConst.ON_ITEM_LOCKED_STATE_CHANGED] = '_OnItemLockedStateChanged', [MessageConst.ON_GEM_DETACH] = 'OnGemDetach', }
...
GemRecastCtrl._RefreshForgeNode = HL.Method(HL.Userdata) << function(self, formula)
    ...
    self.view.formulaHint.text = UIUtils.resolveTextStyle(formula.formulaHint)
    self.view.costItem:InitItem({ id = formula.costItemId, count = formula.costItemNum, }, true)
...
GemRecastCtrl._RefreshGemCellCache = HL.Method(HL.Userdata) << function(self, formula)
    self.m_gemCellCache:Refresh(formula.costGemNum, function(cell, index)
        ...
        if itemInfo then
            ...
            cell.item:InitItem({ id = gemInst.templateId, instId = gemInst.instId, }, true)
...
GemRecastCtrl.OnGemRecast = HL.Method(HL.Table) << function(self, arg)
    ...
    for _, v in pairs(gemInstIds) do
        ...
        if gemInst ~= nil then
            table.insert(items, { id = gemInst.templateId, instId = v, count = 1, })
        end
    end
    if #items <= 0 then
        return
