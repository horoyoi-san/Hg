...
PRTSInvestigateReportCtrl._UpdateData = HL.Method() << function(self)
    ...
    self.m_info = { contentId = itemCfg.contentId, contentType = itemCfg.type == "multi_media" and ContentTypeEnum.Radio or ContentTypeEnum.RichContent, name = itemCfg.name }
end
...
PRTSInvestigateReportCtrl._RefreshAllUI = HL.Method() << function(self)
    ...
    if self.m_showSubmitAni then
        self:_PlaySubmitAni()
        aniWrapper:PlayWithTween("prtsinvestigaterport_in")
    else
        self:_ShowContent()
        aniWrapper:PlayWithTween("prtsstorycolldetailinvesnode_in")
    end
end
...
PRTSInvestigateReportCtrl._OnClickCloseBtn = HL.Method() << function(self)
    ...
    if investCfg then
        Notify(MessageConst.SHOW_SYSTEM_REWARDS, { items = investCfg.rewardItemList })
    end
end
