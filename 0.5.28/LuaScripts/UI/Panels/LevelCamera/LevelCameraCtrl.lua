...
local CAM_SPEED_NO_CURSOR = { ['speed_x'] = 0.9, ['speed_y'] = 0.63 }
LevelCameraCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.TOGGLE_LEVEL_CAMERA_MOVE] = 'ToggleCameraMove', [MessageConst.ON_TOGGLE_INTERACT_OPTION_SCROLL] = 'OnToggleInteractOptionScroll', [MessageConst.ENTER_LEVEL_HALF_SCREEN_PANEL_MODE] = 'EnterLevelHalfScreenPanelMode', [MessageConst.EXIT_LEVEL_HALF_SCREEN_PANEL_MODE] = 'ExitLevelHalfScreenPanelMode', [MessageConst.FAC_ON_DRAG_BEGIN_IN_BATH_MODE] = 'FacOnDragBeginInBathMode', [MessageConst.FAC_ON_DRAG_END_IN_BATH_MODE] = 'FacOnDragEndInBathMode', }
...
LevelCameraCtrl._LoadLevelCamera = HL.Method() << function(self)
    ...
    self.m_camConfig = { luaCustomConfig = camCtrl.transform:GetComponent("LuaCustomConfig") }
...
LevelCameraCtrl._ZoomCamera = HL.Method(HL.Number) << function(self, delta)
    if not self:CanMoveCam() or self.m_disableCameraZoom then
        return
    end
    if LuaSystemManager.facSystem.inTopView then
        LuaSystemManager.facSystem.m_topViewCamCtrl:ZoomCamera(delta, true, function(pos)
