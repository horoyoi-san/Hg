...
MapMarkDetailCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.ON_TRACKING_MAP_MARK] = '_OnMarkTrackingStateChanged', }
...
MapMarkDetailCtrl._SimpleRefreshDetailContent = HL.Method(HL.String) << function(self, markInstId)
    ...
    if markData.missionInfo ~= nil then
        ...
        local tpValid = (markTemplateData.markType == MarkType.CampFire or markTemplateData.markType == MarkType.HUB) and markData.isActive
        if tpValid then
            ...
            self.view.teleportBtn.onClick:AddListener(function()
                if markTemplateData.markType == MarkType.CampFire then
                    Utils.teleportToPosition(markData.levelId, markData.position + Vector3(2, 0, 0), Quaternion.Euler(0, 0, 0))
                else
                    Utils.teleportToPosition(markData.levelId, markData:GetTeleportPosition(), Quaternion.Euler(0, 0, 0))
                end
            end)
        else
            self.view.teleportBtn.gameObject:SetActive(false)
