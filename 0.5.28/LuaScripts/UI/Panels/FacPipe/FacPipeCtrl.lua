...
FacPipeCtrl._RefreshPipeBasicDisplayContent = HL.Method() << function(self)
    ...
    buildingCommonView.speedText.text = string.format("%.1f", 1000 * self.m_buildingInfo.speed)
...
FacPipeCtrl._RefreshPipeItem = HL.Method(HL.Opt(HL.Boolean)) << function(self, forceRefresh)
    ...
    if not string.isEmpty(itemId) then
        self.view.pipeItem:InitItem({ id = itemId, count = self.m_buildingInfo.currentVolume, }, true)
...
FacPipeCtrl._OnDeletePipeButtonClicked = HL.Method() << function(self)
