...
ShopEntryCtrl.InitRecommendation = HL.Method() << function(self)
    local recommendData = GameInstance.player.shopSystem:GetNowRecommendation()
    for k, v in cs_pairs(recommendData) do
        table.insert(self.m_recommendList, v)
    end
    table.sort(self.m_recommendList, function(a, b)
        return a.recommonendType:GetHashCode() < b.recommonendType:GetHashCode()
    end)
end
...
ShopEntryCtrl.InitBeginPool = HL.Method(HL.Any) << function(self, go)
    ...
    if not self.m_cor then
        ...
        local func = function(index)
            ...
            go.rolePoolNode.roleImage1.sprite = self:LoadSprite(UIConst.UI_SPRITE_SHOP_ROLE_IMAGE, self.view.config.RoleBigImage .. id)
            if id == 1 then
                go.rolePoolNode.roleImage2.sprite = self:LoadSprite(UIConst.UI_SPRITE_SHOP_ROLE_IMAGE, self.view.config.RoleBigImage .. 2)
                go.rolePoolNode.roleImage3.sprite = self:LoadSprite(UIConst.UI_SPRITE_SHOP_ROLE_IMAGE, self.view.config.RoleBigImage .. 3)
                indexToId[1] = 1
                indexToId[2] = 2
                indexToId[3] = 3
            end
            if id == 2 then
                go.rolePoolNode.roleImage2.sprite = self:LoadSprite(UIConst.UI_SPRITE_SHOP_ROLE_IMAGE, self.view.config.RoleBigImage .. 3)
                go.rolePoolNode.roleImage3.sprite = self:LoadSprite(UIConst.UI_SPRITE_SHOP_ROLE_IMAGE, self.view.config.RoleBigImage .. 1)
                indexToId[1] = 2
                indexToId[2] = 3
                indexToId[3] = 1
            end
            if id == 3 then
                go.rolePoolNode.roleImage2.sprite = self:LoadSprite(UIConst.UI_SPRITE_SHOP_ROLE_IMAGE, self.view.config.RoleBigImage .. 1)
                go.rolePoolNode.roleImage3.sprite = self:LoadSprite(UIConst.UI_SPRITE_SHOP_ROLE_IMAGE, self.view.config.RoleBigImage .. 2)
                indexToId[1] = 3
                indexToId[2] = 1
                indexToId[3] = 2
            end
...
ShopEntryCtrl.InitUpPool = HL.Method(HL.Any) << function(self, go)
    ...
    go.limitPoolNode.startTxt01.text = string.format(Language.LUA_GACHA_REMAINING_TIME, math.ceil((endTime - curServerTime) / (3600 * 24))) .. "  " .. Language.LUA_SHOP_END_AT .. Utils.appendUTC(os.date("!%m/%d %H:%M", endTime + Utils.getServerTimeZoneOffsetSeconds()))
    go.limitPoolNode.startTxt02.text = string.format(Language.LUA_GACHA_REMAINING_TIME, math.ceil((endTime - curServerTime) / (3600 * 24))) .. "  " .. Language.LUA_SHOP_END_AT .. Utils.appendUTC(os.date("!%m/%d %H:%M", endTime + Utils.getServerTimeZoneOffsetSeconds()))
    ...
    if index == 2 then
        ...
        go.limitPoolNode.upCharImage.sprite = self:LoadSprite(UIConst.UI_SPRITE_SHOP_ROLE_IMAGE, self.view.config.upBG2)
        go.limitPoolNode.charPic.sprite = self:LoadSprite(UIConst.UI_SPRITE_SHOP_ROLE_IMAGE, self.view.config.charPic2)
    end
...
ShopEntryCtrl.InitUpWeaponPool = HL.Method(HL.Any) << function(self, go)
    ...
    go.limitWeaponNode.startTxt02.text = Utils.appendUTC(os.date("!%H:%M", nowPool.openTime + Utils.getServerTimeZoneOffsetSeconds()))
    go.limitWeaponNode.endTxt01.text = os.date("!%m/%d", endTime + Utils.getServerTimeZoneOffsetSeconds())
    go.limitWeaponNode.endTxt02.text = Utils.appendUTC(os.date("!%H:%M", endTime + Utils.getServerTimeZoneOffsetSeconds()))
    go.limitWeaponNode.titleText.text = nowPool.data.name
    if index > 2 then
        index = index - 2
    end
    if index == 2 then
        go.limitWeaponNode.upWeaponPoolImage.sprite = self:LoadSprite(UIConst.UI_SPRITE_SHOP_ROLE_IMAGE, self.view.config.upWeaponBG2)
    end
