...
CharInfoAttributeCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.P_CHAR_INFO_SELECT_CHAR_CHANGE] = 'OnSelectCharChange', [MessageConst.P_CHAR_INFO_PAGE_CHANGE] = 'OnSelectTabChange', [MessageConst.P_ON_CHAR_FORMATION_CLOSE_SKILL_TIP] = '_OnSkillTipClose', [MessageConst.ON_UNLOCK_SKILL] = '_OnUnlockSkill', [MessageConst.CHAR_NORMAL_SKILL_CHANGE] = '_OnCharNormalSkillChange', }
...
CharInfoAttributeCtrl._InitActionEvent = HL.Method() << function(self)
    ...
    overviewNode.levelUpButton.onClick:AddListener(function()
        ...
        if isRPGDungeon then
            self:Notify(MessageConst.SHOW_TOAST, Language.LUA_RPG_DUNGEON_FORBID_CHAR_UPGRADE)
            return
        end
        self:Notify(MessageConst.P_CHAR_INFO_PAGE_CHANGE, { pageType = UIConst.CHAR_INFO_PAGE_TYPE.UPGRADE })
    end)
...
CharInfoAttributeCtrl._RefreshAttributePanel = HL.Method(HL.Table, HL.Number, HL.Opt(HL.Number, HL.Number, HL.Number)) << function(self, charInfo, mainControlTab, selectedEquipInstId, compareEquipInstId, slotIndex)
...
CharInfoAttributeCtrl._RefreshOverviewNode = HL.Method(HL.Table) << function(self, charInfo)
    ...
    basicInfoNode.elementIcon.sprite = self:LoadSprite(UIConst.UI_SPRITE_CHAR_ELEMENT, elementSpriteName)
    basicInfoNode.elementIconShadow.sprite = self:LoadSprite(UIConst.UI_SPRITE_CHAR_ELEMENT, elementSpriteName)
    local proSpriteName = UIConst.UI_CHAR_PROFESSION_PREFIX .. charData.profession:ToInt()
    basicInfoNode.imageProShadow.sprite = self:LoadSprite(UIConst.UI_SPRITE_CHAR_PROFESSION, proSpriteName)
    basicInfoNode.imagePro.sprite = self:LoadSprite(UIConst.UI_SPRITE_CHAR_PROFESSION, proSpriteName)
    ...
    self.m_talentList:Refresh(row, function(talentList, rowIndex)
        ...
        cellCache:Refresh(cellNum, function(cell, columnIndex)
            ...
            cell.button.onClick:AddListener(function()
                local talentInfo = { talentData = talentData, nextBreakStage = talent.nextBreakStage, transform = cell.transform, unlock = unlock, }
...
CharInfoAttributeCtrl._OnClickDetailButton = HL.Method() << function(self)
    ...
    if phase then
        phase:CreateCharInfoPanel(PanelId.CharInfoFullAttribute, { initCharInfo = self.m_charInfo, mainControlTab = self.m_curMainControlTab, phase = self.m_phase })
    end
end
CharInfoAttributeCtrl._OnSkillTipClose = HL.Method(HL.Opt(HL.Any)) << function(self, arg)
    self.view.overviewNode.charSkillNode:RefreshSkillSelect()
