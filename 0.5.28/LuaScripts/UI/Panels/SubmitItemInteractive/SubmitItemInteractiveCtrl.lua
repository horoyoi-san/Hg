...
SubmitItemInteractiveCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.ON_SUBMIT_ITEM] = 'OnSubmitItem', }
...
SubmitItemInteractiveCtrl.OnCreate = HL.Override(HL.Any) << function(self, arg)
    ...
    self.m_selectItemBundle = { id = "", count = 0, instId = 0 }
...
SubmitItemInteractiveCtrl._OnSelectItem = HL.Method(HL.Table, HL.Any) << function(self, itemBundle, cell)
    ...
    table.insert(self.m_submitItemsSelect, { id = itemBundle.id, count = 1, instId = itemBundle.instId })
    self:_UpdateCount()
end
SubmitItemInteractiveCtrl._UpdateSelectText = HL.Method() << function(self)
    ...
    if (hasSelectItem) then
        ...
        self.view.textSelect.text = UIUtils.resolveTextStyle(string.format(Language.LUA_SUBMIT_ITEM_SELECT_TIP, #self.m_submitItemsSelect, submitItemData.name, rarityColorStr))
    end
end
