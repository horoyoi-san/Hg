...
RemoteCommCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.P_REMOTE_COMM_PAUSE] = 'RemoteCommPause', [MessageConst.P_REMOTE_COMM_RESUME] = 'RemoteCommResume', }
...
RemoteCommCtrl._RefreshCellWithAnim = HL.Method(HL.Table, HL.Boolean, HL.Boolean, HL.Opt(HL.Number, HL.String)) << function(self, cell, outAnim, inAnim, inDelay, anim)
    local animationWrapper = cell.animationWrapper
...
RemoteCommCtrl._RefreshSingleActor = HL.Method(HL.Table, HL.Any) << function(self, cell, actorId)
    if string.isEmpty(actorId) then
        ...
        if res then
            cell.char.sprite = self:LoadSprite(UIConst.UI_SPRITE_CHAR_REMOTE_ICON, actorImageData.bustPath)
        end
    end
end
RemoteCommCtrl._RefreshSingleBG = HL.Method(HL.Table, HL.Any) << function(self, cell, imageId)
    if imageId then
        ...
        local sprite
        if res then
            sprite = self:LoadSprite(UIConst.UI_SPRITE_VIDEO_COVER, imageId)
        else
            sprite = self:LoadSprite(imageId)
        end
        cell.banner.sprite = sprite
    end
end
RemoteCommCtrl._IsVideo = HL.Method().Return(HL.Boolean, HL.Any) << function(self)
    local middleId = self.m_singleData.middleId
    local videoKey = "RemoteComm/" .. middleId
...
RemoteCommCtrl._RefreshMidContent = HL.Method() << function(self)
    ...
    if isVideo then
        ...
        self.view.videoImage.player.statusChangeCallback = function(status)
            ...
            if status == CS.CriWare.CriMana.Player.Status.Playing and movieInfo then
                ...
                if not string.isEmpty(self.m_audioId) then
                    self:_DoPlayAudio("", self.m_audioId)
                    self.m_audioId = ""
                end
            end
        end
    ...
    elseif isActor then
        local sprite = self:LoadSprite(UIConst.UI_SPRITE_CHAR_REMOTE_ICON_700, actorImageData.bustPath)
        self.view.charImageEx.sprite = sprite
    else
        self.view.bgImage.sprite = self:LoadSprite(middleId)
    end
    local wrapper
    if isVideo then
...
RemoteCommCtrl._TryRefreshCell = HL.Method(HL.Table, HL.Any, HL.Any, HL.Function, HL.Opt(HL.Number, HL.String)) << function(self, cell, actorId, lastActorId, refreshFunc, inDelay, anim)
    if lastActorId then
        if actorId then
...
RemoteCommCtrl._DoPlayAudio = HL.Method(HL.String, HL.String) << function(self, voiceId, audioId)
...
RemoteCommCtrl._TryPlayAudio = HL.Method() << function(self)
    ...
    if not self.m_inited then
        ...
        if isVideo then
            ...
            self:_DoPlayAudio(voiceId, audioId)
        end
    end
end
RemoteCommCtrl._RefreshMiddle = HL.Method() << function(self)
    ...
    elseif lastMid ~= self.m_singleData.middleId then
        local videoKey = "RemoteComm/" .. lastMid
        ...
        if lastWrapper.curState ~= UIConst.UI_ANIMATION_WRAPPER_STATE.Out then
            ...
            lastWrapper:PlayOutAnimation(function()
                if needPlayVoice then
                    self:_TryPlayAudio()
                elseif not string.isEmpty(audioId) then
                    self:_DoPlayAudio("", audioId)
                end
                self:_RefreshMidContent()
            end)
        end
...
RemoteCommCtrl.OnShow = HL.Override() << function(self)
    ...
    self:GetAnimationWrapper():PlayInAnimation(function()
        if not string.isEmpty(self.m_audioId) or not string.isEmpty(self.m_voiceId) then
            self:_DoPlayAudio(self.m_voiceId, self.m_audioId)
            self.m_voiceId = ""
            self.m_audioId = ""
        end
    end)
