...
FacPipeConnectorCtrl._GetViewIndexByConnectorPortIndex = HL.Method(HL.Number, HL.Boolean).Return(HL.Number) << function(self, portLuaIndex, isIn)
    if isIn then
...
FacPipeConnectorCtrl._RefreshConnectorItemState = HL.Method(HL.Number, HL.String) << function(self, index, itemId)
    ...
    if itemId ~= lastItemId then
        if string.isEmpty(itemId) then
            ...
            viewItem:InitItem({ id = itemId, count = 1, isInfinite = FactoryUtils.isItemInfiniteInFactoryDepot(itemId), }, true)
...
FacPipeConnectorCtrl._InitAnimDataMap = HL.Method() << function(self)
    self.m_inBindingAnimMap = { { decoAnimWrapper = self.view.arrowDecoAnimationIn1, decoAnimName = "facconnector_arrow02", conveyorAnimWrapper = self.view.conveyorAnimationIn1, conveyorAnimName = "facpipeconnector_arrow", }, { decoAnimWrapper = self.view.arrowDecoAnimationIn2, decoAnimName = "facconnector_arrow", conveyorAnimWrapper = self.view.conveyorAnimationIn2, conveyorAnimName = "facpipeconnector_arrow", }, }
    self.m_outBindingAnimMap = { { decoAnimWrapper = self.view.arrowDecoAnimationOut1, decoAnimName = "facconnector_arrow03", conveyorAnimWrapper = self.view.conveyorAnimationOut1, conveyorAnimName = "facpipeconnector_arrow02", }, { decoAnimWrapper = self.view.arrowDecoAnimationOut2, decoAnimName = "facconnector_arrow01", conveyorAnimWrapper = self.view.conveyorAnimationOut2, conveyorAnimName = "facpipeconnector_arrow02", }, }
    self.m_inItemAnimMap = { { animationNode = self.view.itemAnimationIn1, animationName = "connector_facpipe_changed", }, { animationNode = self.view.itemAnimationIn2, animationName = "connector_facpipe_changed_3", } }
    self.m_outItemAnimMap = { { animationNode = self.view.itemAnimationOut1, animationName = "connector_facpipe_changed_1", }, { animationNode = self.view.itemAnimationOut2, animationName = "connector_facpipe_changed_2", }, }
end
