...
FormulaCtrl.ShowFormula = HL.StaticMethod(HL.Table) << function(args)
    ...
    self:_ShowFormula(buildingId, nodeId)
end
FormulaCtrl._ShowFormula = HL.Method(HL.String, HL.Number) << function(self, buildingId, nodeId)
...
FormulaCtrl._OnUpdateCell = HL.Method(HL.Table, HL.Number) << function(self, cell, index)
    ...
    formulaCell.time.text = string.format(Language["LUA_CRAFT_CELL_STANDARD_TIME"], FactoryUtils.getCraftTimeStr(craftInfo.time))
