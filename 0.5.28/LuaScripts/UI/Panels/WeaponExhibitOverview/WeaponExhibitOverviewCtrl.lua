...
WeaponExhibitOverviewCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.ON_ITEM_LOCKED_STATE_CHANGED] = 'OnItemLockedStateChanged', }
...
WeaponExhibitOverviewCtrl._InitActionEvent = HL.Method() << function(self)
    ...
    self.view.btnDocument.onClick:AddListener(function()
        self:Notify(MessageConst.WEAPON_EXHIBIT_PAGE_CHANGE, { pageType = UIConst.WEAPON_EXHIBIT_PAGE_TYPE.DOCUMENT, })
    end)
    self.view.btnHideUI.onClick:AddListener(function()
        self:_ToggleUI(false)
    end)
    self.view.btnPotential.onClick:AddListener(function()
        self:Notify(MessageConst.WEAPON_EXHIBIT_PAGE_CHANGE, { pageType = UIConst.WEAPON_EXHIBIT_PAGE_TYPE.POTENTIAL, })
    end)
    self.view.btnFullSkill.onClick:AddListener(function()
        UIManager:Open(PanelId.WeaponSkillDetail, { weaponInstId = self.m_weaponInfo.weaponInstId, })
    end)
end
...
WeaponExhibitOverviewCtrl._OnClickUpgradeButton = HL.Method() << function(self)
    ...
    local isTrailCard = CharInfoUtils.checkIsCardInTrail(weaponExhibitInfo.weaponInst.equippedCharServerId)
    if isTrailCard then
        Notify(MessageConst.SHOW_TOAST, Language.LUA_CHAR_INFO_WEAPON_UPGRADE_TRAIL_FORBID)
        return
    end
    self:Notify(MessageConst.WEAPON_EXHIBIT_PAGE_CHANGE, { pageType = UIConst.WEAPON_EXHIBIT_PAGE_TYPE.UPGRADE, })
end
...
WeaponExhibitOverviewCtrl._OnClickGemTab = HL.Method() << function(self)
    self:Notify(MessageConst.WEAPON_EXHIBIT_PAGE_CHANGE, { pageType = UIConst.WEAPON_EXHIBIT_PAGE_TYPE.GEM, })
end
...
WeaponExhibitOverviewCtrl._RefreshOverviewPanel = HL.Method(HL.Number, HL.String) << function(self, weaponInstId, weaponTemplateId)
    ...
    self.view.potentialStar:InitWeaponPotentialStar(weaponExhibitInfo.weaponInst.refineLv, { showMaxPotentialHint = true, })
...
WeaponExhibitOverviewCtrl._RefreshAttributeCell = HL.Method(HL.Table, HL.Table) << function(self, cell, attributeInfo)
    ...
    cell.attribute.attributeIcon.sprite = self:LoadSprite(UIConst.UI_SPRITE_ATTRIBUTE_ICON, attributeInfo.iconName)
end
    ...
    self:_StartCoroutine(function()
        self.m_rotateTickKey = LuaUpdate:Add("LateTick", function(deltaTime)
            ...
            if stickValue.x ~= 0 then
                phase:RotateWeapon(stickValue.x * self.view.config.CONTROLLER_ROTATE_SENSITIVITY)
            end
        end)
    end)
