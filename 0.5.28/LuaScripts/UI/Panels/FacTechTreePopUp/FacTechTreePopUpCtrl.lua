...
FacTechTreePopUpCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.HIDE_TECH_TREE_POP_UP] = 'HideUI', [MessageConst.CLOSE_TECH_TREE_POP_UP] = 'CloseUI', }
...
FacTechTreePopUpCtrl._ShowUnlock = HL.Method() << function(self)
    ...
    self.view.desc.text = UIUtils.resolveTextStyle(techData.unlockDesc)
end
FacTechTreePopUpCtrl._ShowLevelUp = HL.Method() << function(self)
    ...
    local spBuildingData = Tables.factorySpecialBuildingTable:GetValue(buildingId)
    local find, charCount = spBuildingData.levelData:TryGetValue(level)
    node.char.gameObject:SetActiveIfNecessary(find and level > 1)
    if find and level > 1 then
        node.char.lText.text = GameInstance.player.facCharacterSystem:GetCharSoltCount(buildingId, level - 1)
        node.char.rText.text = charCount
    end
    local showContract = false
    local curCount = 0
    local lastCount = 0
    if buildingData.type == GEnums.FacBuildingType.Trader then
        for _, contractData in pairs(Tables.contractTable) do
            if contractData.traderLevel <= level then
                curCount = curCount + 1
            end
            if contractData.traderLevel <= level - 1 then
