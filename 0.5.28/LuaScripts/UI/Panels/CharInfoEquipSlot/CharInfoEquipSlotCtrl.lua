...
CharInfoEquipSlotCtrl._InitActionEvent = HL.Method() << function(self)
    ...
    self.view.commonEmptyButton.onClick:AddListener(function()
        self:Notify(MessageConst.P_CHAR_INFO_EMPTY_BUTTON_CLICK)
    end)
...
CharInfoEquipSlotCtrl._RefreshTacticalSlot = HL.Method(HL.Table, HL.Opt(HL.Boolean)) << function(self, cell, isSelectSlot)
    ...
    cell.numText.text = Utils.getBagItemCount(charInst.tacticalItemId)
    cell.iconRaw.gameObject:SetActive(hasValue)
    local itemCfg = Tables.itemTable:GetValue(charInst.tacticalItemId)
    cell.iconRaw.sprite = self:LoadSprite(UIConst.UI_SPRITE_ITEM_BIG, itemCfg.iconId)
    UIUtils.setItemRarityImage(cell.colorDeco, itemCfg.rarity)
    cell.count.text = Utils.getBagItemCount(charInst.tacticalItemId)
end
CharInfoEquipSlotCtrl._RefreshEquipSlot = HL.Method(HL.Table, HL.Opt(HL.Number, HL.Boolean, HL.Boolean)) << function(self, cell, equipInstId, isSelectSlot, needRefresh)
    if not cell then
        return
    end
    ...
    if hasEquip then
        ...
        local spritePath = UIUtils.getSpritePath(UIConst.UI_SPRITE_ITEM_BIG, data.iconId)
        cell.iconRaw.texture = self.loader:LoadTexture(spritePath)
        cell.equipmentLogo.gameObject:SetActive(false)
        if needRefresh then
...
CharInfoEquipSlotCtrl.OnSelectTacticalItemChange = HL.Method(HL.Table) << function(self, arg)
    ...
    self.view.tacticalNode.tacticalIcon.sprite = self:LoadSprite(UIConst.UI_SPRITE_ITEM_BIG, itemId)
...
CharInfoEquipSlotCtrl.OnSelectEquipChange = HL.Method(HL.Table) << function(self, arg)
    ...
    for _, info in ipairs(scSubAttrFromShowList) do
        comboScSubAttrShowDict[info.attributeType] = { fromInfo = info, toInfo = AttributeUtils.generateEmptyAttributeShowInfo(info.attributeType), }
    end
    for _, info in ipairs(scSubAttrToShowList) do
        ...
        if comboInfo then
            ...
            comboScSubAttrShowDict[info.attributeType] = { fromInfo = AttributeUtils.generateEmptyAttributeShowInfo(info.attributeType), toInfo = info }
        end
    end
    local comboShowList = {}
...
CharInfoEquipSlotCtrl._RefreshAttrCell = HL.Method(HL.Table, HL.Table, HL.Table) << function(self, cell, fromAttrInfo, toAttrInfo)
    ...
    if hasIconName then
        cell.icon.sprite = self:LoadSprite(UIConst.UI_SPRITE_ATTRIBUTE_ICON, fromAttrInfo.iconName)
    end
...
CharInfoEquipSlotCtrl._OnClickSlotCell = HL.Method(HL.Number) << function(self, slotIndex)
    ...
    if isTrail then
        Notify(MessageConst.SHOW_TOAST, Language.LUA_CHAR_INFO_IN_TRIAL_FORBID_INTERACT_TOAST)
        return
    end
    ...
    self:Notify(MessageConst.P_CHAR_INFO_SELECT_EQUIP_SLOT_CHANGE, { slotIndex = slotIndex })
    self:Notify(MessageConst.P_CHAR_INFO_EQUIP_SECOND_OPEN)
