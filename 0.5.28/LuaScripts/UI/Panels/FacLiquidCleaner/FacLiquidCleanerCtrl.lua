...
local InfoState = { None = "None", Processing = "Processing", Finished = "Finished", Empty = "Empty", Paused = "Paused", }
...
FacLiquidCleanerCtrl.OnCreate = HL.Override(HL.Any) << function(self, arg)
    ...
    self.view.facCacheRepository:InitFacCacheRepository({ cache = self.m_buildingInfo.fluidCache, isInCache = true, isFluidCache = true, cacheIndex = 1, slotCount = 1, fakeFormulaDataList = FactoryUtils.getBuildingCrafts(self.m_buildingInfo.buildingId) })
...
FacLiquidCleanerCtrl._RefreshCleanerTipsVisibleState = HL.Method() << function(self)
    local needShowTips = self.m_isPipeBlocked and string.isEmpty(self.m_buildingInfo.consumeItemId)
...
FacLiquidCleanerCtrl._ClearCleanerItemData = HL.Method() << function(self)
    ...
    self.m_sewageItemData = { id = "", count = 0, }
    self.m_isItemDirty = true
end
FacLiquidCleanerCtrl._RefreshCleanerInfo = HL.Method() << function(self)
    ...
    if string.isEmpty(self.m_sewageItemData.id) then
        state = InfoState.Empty
    else
        if self.m_sewageItemData.count == 0 then
            state = InfoState.Finished
        else
            state = self.view.buildingCommon.lastState == GEnums.FacBuildingState.Normal and InfoState.Processing or InfoState.Paused
        end
    end
