...
SkillUpgradePopUpCtrl._ShowShipSkillUpgrade = HL.Method(HL.Number, HL.String) << function(self, charInstId, nodeId)
    ...
    shipSkillNode.icon.sprite = self:LoadSprite(UIConst.UI_SPRITE_SS_SKILL_ICON, curSkillCfg.icon)
end
SkillUpgradePopUpCtrl._ShowPassiveSkillUpgrade = HL.Method(HL.Number, HL.String) << function(self, charInstId, nodeId)
    ...
    passiveSkillNode.desc.text = UIUtils.resolveTextStyle(nodeDesc)
    passiveSkillNode.icon.sprite = self:LoadSprite(UIConst.UI_SPRITE_SKILL_ICON, curSkillInfo.iconId)
end
