...
FacPowerGateCtrl._RefreshPowerGate = HL.Method(HL.Number) << function(self, fromNodeId)
    ...
    local _, fromPowerGateMapCfg = GameInstance.world.curLevel.levelData.factoryPredefineData.powerGateTable:TryGetValue(fromInstKey)
...
FacPowerGateCtrl._ShowToMap = HL.Method() << function(self)
    local isMapUnlock = self.m_toIconId and self.m_toMapId and GameInstance.player.mapManagerOdd:CheckLevelUnlock(self.m_toMapId) and GameInstance.player.mapManagerOdd:HasMarkIcon(self.m_toIconId)
