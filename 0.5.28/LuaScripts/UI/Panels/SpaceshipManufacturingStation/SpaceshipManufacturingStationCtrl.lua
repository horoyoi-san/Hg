...
SpaceshipManufacturingStationCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.SPACESHIP_ON_SYNC_ROOM_STATION] = "OnSpaceshipRoomStationSync", [MessageConst.ON_SPACESHIP_MANUFACTURING_STATION_START] = "OnSpaceshipManufacturingStationStart", [MessageConst.ON_SPACESHIP_MANUFACTURING_STATION_SYNC] = "OnSpaceshipManufacturingStationSync", [MessageConst.ON_SPACESHIP_MANUFACTURING_STATION_COLLECT] = "OnSpaceshipManufacturingStationCollect", [MessageConst.ON_SPACESHIP_MANUFACTURING_STATION_CANCEL] = "OnSpaceshipManufacturingStationCancel", }
...
SpaceshipManufacturingStationCtrl._InitFormulaList = HL.Method() << function(self)
    ...
    formulaData.sortOptions = { { name = Language.LUA_FAC_CRAFT_SORT_1, keys = { "isWorkingSortId", "isUnlockSortId", "sortId" } }, { name = Language.LUA_FAC_CRAFT_SORT_2, keys = { "isWorkingSortId", "rarity", "sortId" } }, }
...
SpaceshipManufacturingStationCtrl._ProcessFormulaTabListData = HL.Method().Return(HL.Table) << function(self)
    ...
    for _, formulaData in pairs(Tables.spaceshipManufactureFormulaTable) do
        ...
        local formulaDataUnit = { formulaId = formulaData.id, itemId = outcomeItemData.id, roomId = self.m_roomId, isWorking = isWorking, isUnlock = isUnlock, rarity = formulaData.rarity, sortId = formulaData.sortId, isWorkingSortId = isWorking and 0 or 1, inUnlockSortId = isUnlock and 1 or 0, }
    ...
    for showType, formulaDataList in pairs(formulaShowType2Data) do
        ...
        table.insert(formulaTabListData, { tabName = showingTypeCfgData.name, iconName = showingTypeCfgData.icon, sortId = showType:GetHashCode(), list = formulaDataList, })
    end
    table.insert(formulaTabListData, { tabName = Language.LUA_SPACESHIP_ROOM_FORMULA_LIST_TAB_ALL_DESC, iconName = "icon_type_all", sortId = 0, list = allFormulaList, })
...
SpaceshipManufacturingStationCtrl._TickFormulaPanel = HL.Method() << function(self)
    ...
    if string.isEmpty(remainFormulaId) or not isProducing or self.m_showingFormulaList and self.m_curSelectFormulaId ~= remainFormulaId then
        return
    end
    self:_RefreshNumberSelector()
...
SpaceshipManufacturingStationCtrl._RefreshFormulaOutcomeInfo = HL.Method() << function(self)
    local spaceship = GameInstance.player.spaceship
    local formulaId = self.m_showingFormulaList and self.m_curSelectFormulaId or spaceship:GetManufacturingStationRemainFormulaId(self.m_roomId)
    ...
    if hasFormulaId then
        ...
        self.view.formulaStateTxt.text = string.format(Language.LUA_SPACESHIP_MANUFACTURING_STATION_OWN_OUTCOME_FORMAT, ownCount)
...
SpaceshipManufacturingStationCtrl._RefreshNumberSelector = HL.Method() << function(self)
    ...
    if selectUnlock then
        local remainFormulaSelectedShowCount = remainProduceCount > 0 and math.max(remainProduceCount + self.m_diffBetweenSelectAndRemain, 1) or remainProduceCount
    ...
    self.view.numberSelector.gameObject:SetActiveIfNecessary(self.m_showingFormulaList and selectUnlock or not self.m_showingFormulaList and hasRemainFormula)
end
SpaceshipManufacturingStationCtrl._RefreshFormulaTimeInfoBySelected = HL.Method() << function(self)
    local spaceship = GameInstance.player.spaceship
    local machineCapacity = spaceship:GetManufacturingStationCapacity(self.m_roomId)
    local formulaData = Tables.spaceshipManufactureFormulaTable[self.m_curSelectFormulaId]
    local roomProduceRate = spaceship:GetRoomProduceRate(self.m_roomId, GEnums.SpaceshipRoomAttrType.RoomProduceRate)
    local selectUnlock = spaceship:IsManufactureFormulaUnlocked(self.m_roomId, self.m_curSelectFormulaId)
    self.view.schedule.fillAmount = 0
    self.view.countdownTxt.text = UIUtils.getLeftTimeToSecondFull(selectUnlock and (formulaData.totalProgress / roomProduceRate) or 0)
    self.view.capacityTxt.text = string.format(Language.LUA_SPACESHIP_MANUFACTURING_STATION_CAPACITY_FORMAT, self.m_curSelectNumber * formulaData.perCapacity, machineCapacity)
    self.view.timeTxt.text = UIUtils.getLeftTimeToSecondFull(self.m_curSelectNumber * formulaData.totalProgress / roomProduceRate)
...
SpaceshipManufacturingStationCtrl._RefreshFormulaTimeInfoByRemain = HL.Method() << function(self)
    ...
    if hasRemainFormula then
        ...
        self.view.formulaStateTxt.text = string.format(Language.LUA_SPACESHIP_MANUFACTURING_STATION_OWN_OUTCOME_FORMAT, Utils.getItemCount(formulaData.outcomeItemId))
        self.view.capacityTxt.text = string.format(Language.LUA_SPACESHIP_MANUFACTURING_STATION_CAPACITY_FORMAT, panelRemainCount * formulaData.perCapacity, machineCapacity)
        self.view.schedule.fillAmount = remainProduceCount > 0 and realProgress / formulaData.totalProgress or 0
        self.view.countdownTxt.text = UIUtils.getLeftTimeToSecondFull(panelRemainCount > 0 and curLeftProgress / roomProduceRate or 0)
        self.view.timeTxt.text = UIUtils.getLeftTimeToSecondFull(panelRemainCount > 0 and (curLeftProgress + formulaData.totalProgress * math.max(0, panelRemainCount - 1)) / roomProduceRate or 0)
        self.view.suspendImg.gameObject:SetActiveIfNecessary(isShutDown)
    else
        self.view.schedule.fillAmount = 0
        self.view.timeTxt.text = UIUtils.getLeftTimeToSecondFull(0)
        self.view.countdownTxt.text = UIUtils.getLeftTimeToSecondFull(0)
        self.view.capacityTxt.text = string.format(Language.LUA_SPACESHIP_MANUFACTURING_STATION_CAPACITY_FORMAT, 0, machineCapacity)
...
SpaceshipManufacturingStationCtrl._RefreshFormulaBottomState = HL.Method() << function(self)
    ...
    elseif self.m_showingFormulaList and selectUnlock or self.m_diffBetweenSelectAndRemainDirty then
        ...
        local showHint = spaceship:GetManufacturingStationRemainProduceCount(self.m_roomId) > 0 and not isRemainFormulaSelected and (isProducing or hasProduct)
        self.view.confirmHint.gameObject:SetActiveIfNecessary(showHint)
    elseif not self.m_showingFormulaList and hasProduct then
        ...
        if self.m_showingFormulaList then
            ...
            self.view.stateTxt.text = string.format(Language.LUA_SPACESHIP_MANUFACTURING_STATION_FORMULA_UNLOCK_CONDITION_FORMAT, formulaCfg.level)
        else
...
SpaceshipManufacturingStationCtrl._OnNumberSelectorChange = HL.Method(HL.Number, HL.Boolean) << function(self, curNumber, isChangeByBtn)
    self.m_curSelectNumber = curNumber
    local spaceship = GameInstance.player.spaceship
    local remainFormulaId = spaceship:GetManufacturingStationRemainFormulaId(self.m_roomId)
    if (not self.m_showingFormulaList or self.m_showingFormulaList and self.m_curSelectFormulaId == remainFormulaId) and isChangeByBtn then
        local remainProduceCount = spaceship:GetManufacturingStationRemainProduceCount(self.m_roomId)
...
SpaceshipManufacturingStationCtrl._RefreshOverCapacity = HL.Method(HL.String) << function(self, formulaId)
    ...
    if overCapacity then
        ...
        self.view.overCapacityTxt.text = reachMachineCapacity and Language.LUA_SPACESHIP_MANUFACTURING_STATION_OVER_CAPACITY or Language.LUA_SPACESHIP_MANUFACTURING_STATION_NEARLY_OVER_CAPACITY
...
SpaceshipManufacturingStationCtrl._RefreshPanelTopNode = HL.Method() << function(self)
    ...
    if self.m_showingFormulaList then
        local spaceship = GameInstance.player.spaceship
        local rate = spaceship:GetRoomProduceRate(self.m_roomId, GEnums.SpaceshipRoomAttrType.RoomProduceRate)
        local txt = string.format(Language.LUA_SPACESHIP_MANUFACTURING_STATION_PRODUCE_RATE_FORMAT, rate)
        self.view.spaceshipRoomCommonBg:RefreshTopInfoNode(txt)
    end
end
...
SpaceshipManufacturingStationCtrl._OnFormulaChange = HL.Method() << function(self)
    if GameInstance.player.spaceship:GetManufacturingStationRemainFormulaId(self.m_roomId) == self.m_curSelectFormulaId then
        self:_RefreshFormulaInfoByRemain()
    else
...
SpaceshipManufacturingStationCtrl._ShowCancelFormulaToast = HL.Method() << function(self)
    local spaceship = GameInstance.player.spaceship
    local _, count = spaceship:GetManufacturingStationProduct(self.m_roomId)
    local content = count > 0 and Language.LUA_SPACESHIP_MANUFACTURING_STATION_CANCEL_FORMULA_COMMON_WITH_PRODUCT_DESC or Language.LUA_SPACESHIP_MANUFACTURING_STATION_CANCEL_FORMULA_COMMON_WITHOUT_PRODUCT_DESC
...
SpaceshipManufacturingStationCtrl._ShowExchangeFormulaToast = HL.Method() << function(self)
    ...
    Notify(MessageConst.SHOW_POP_UP, {
        ...
        onConfirm = function()
            GameInstance.player.spaceship:ManufacturingStationChangeFormula(self.m_roomId, self.m_curSelectFormulaId, self.m_curSelectNumber)
        end,
        onCancel = function()
        end
    })
...
SpaceshipManufacturingStationCtrl._OnMoreInfoBtnClick = HL.Method() << function(self)
    local roomInstData = Tables.spaceshipRoomInsTable[self.m_roomId]
    local roomTypeData = Tables.spaceshipRoomTypeTable[roomInstData.roomType]
    Notify(MessageConst.SHOW_COMMON_TIPS, { text = roomTypeData.extraDesc1, transform = self.view.moreInfoBtn.transform, posType = UIConst.UI_TIPS_POS_TYPE.LeftTop, })
end
...
SpaceshipManufacturingStationCtrl.OnSpaceshipManufacturingStationCollect = HL.Method(HL.Any) << function(self, args)
    ...
    Notify(MessageConst.SHOW_SYSTEM_REWARDS, { title = Language.LUA_SPACESHIP_MANUFACTURING_STATION_COLLECT_OUTCOME, items = { { id = itemId, count = count }, } })
    self:_RefreshFormulaInfoByRemain()
end
