...
WikiWeaponSkillCtrl._OnPhaseItemBind = HL.Override() << function(self)
    self.view.top:InitWikiTop({ phase = self.m_phase, panelId = PANEL_ID, wikiEntryShowData = self.m_curWikiEntryShowData, forceShowBackBtn = true, })
end
    ...
    if not self.m_getSkillItemCell then
        ...
        self.view.left.scrollListLeft.onUpdateCell:AddListener(function(object, csIndex)
            ...
            if showData.isUnlocked then
                self:_SetCellSelected(cell, LuaIndex(csIndex) == self.m_selectedIndex)
            else
...
WikiWeaponSkillCtrl._SetSelectedIndex = HL.Method(HL.Number) << function(self, selectedIndex)
    ...
    self:_SetCellSelected(lastSelectedCell, false)
    ...
    self:_SetCellSelected(selectedCell, true)
    self.m_curWeaponSkillShowData = self.m_weaponSkillShowDataList[selectedIndex]
    self:_RefreshCenter()
end
WikiWeaponSkillCtrl._SetCellSelected = HL.Method(HL.Table, HL.Boolean) << function(self, cell, isSelected)
    if not cell then
        return
    end
...
WikiWeaponSkillCtrl._RefreshCenter = HL.Method() << function(self)
    ...
    if not self.m_getEffectCell then
        ...
        self.view.center.scrollListSkillEffect.onUpdateCell:AddListener(function(object, csIndex)
            ...
            cell.txtDec.text = UIUtils.resolveTextStyle(desc)
        end)
    end
    self.view.center.scrollListSkillEffect:UpdateCount(skillPatchDataCount)
    self.m_weaponList = {}
    for _, weaponData in ipairs(self.m_curWeaponSkillShowData.weaponDataList) do
        ...
        if itemData and weaponBasicData then
            local weapon = { id = weaponId, isUnlocked = weaponData.isUnlocked, weaponType = weaponBasicData.weaponType:ToInt(), rarity = itemData.rarity, unlockedState = weaponData.isUnlocked and 1 or 0 }
