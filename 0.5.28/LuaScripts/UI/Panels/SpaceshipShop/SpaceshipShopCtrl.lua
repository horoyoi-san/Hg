...
SpaceshipShopCtrl.RefreshSpaceShipSheetTabs = HL.Method(HL.String, HL.Opt(HL.String)) << function(self, curGroupId, shopId)
    ...
    for goodsId, goodsData in pairs(shopData.goodsDic) do
        ...
        if true then
            ...
            local info = { id = goodsId, rarity = itemTableData.rarity, price = goodsTableData.price * goodsData.discount, sortId = goodsTableData.sortId, isLocked = not isUnlocked, }
            if self.m_needShowUnlock then
                info.isLocked = true
            end
...
SpaceshipShopCtrl._GetMoneyTimeText = HL.Method(HL.Number).Return(HL.Any) << function(self, leftTime)
    if leftTime > 24 * 3600 then
        return tostring(UIUtils.resolveTextStyle(string.format(Language.LUA_SPACE_SHOP_REFRESH_TWO_DAY, math.floor(leftTime / (24 * 3600)) + 1)))
    elseif leftTime < 24 * 3600 and leftTime > 3600 then
        return tostring(UIUtils.resolveTextStyle(string.format(Language.LUA_SPACE_SHOP_REFRESH_ONE_DAY, math.floor(leftTime / 3600))))
    else
        return tostring(UIUtils.resolveTextStyle(string.format(Language.LUA_SPACE_SHOP_REFRESH_AT_ONCE, math.max(1, math.floor(leftTime / 60)))))
    end
end
...
SpaceshipShopCtrl.OnAfterBuyItemSucc = HL.Override() << function(self)
    ...
    if sellOut == allCount and nowIndex <= groupData.shopIdList.Count - 2 then
        ...
        Notify(MessageConst.SHOW_POP_UP, {
            content = Language.LUA_SPACE_SHOP_UNLOCK_NEXT_TIPS,
            hideCancel = true,
            onConfirm = function()
