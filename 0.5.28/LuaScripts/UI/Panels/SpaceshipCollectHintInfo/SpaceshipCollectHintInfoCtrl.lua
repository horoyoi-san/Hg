    ...
    for roomId, roomInfo in pairs(spaceship.rooms) do
        ...
        if succ and products.Count > 0 then
            ...
            for k, v in pairs(products) do
                ...
                table.insert(productsLua, { id = k, count = v, sortId1 = iData.sortId1, sortId2 = iData.sortId2, })
            end
            table.sort(productsLua, Utils.genSortFunction(UIConst.COMMON_ITEM_SORT_KEYS))
            table.insert(rooms, { id = roomId, room = roomInfo, data = data, sortId = data.sortId, products = productsLua, })
        end
    end
