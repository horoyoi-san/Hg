...
local GameMechanicsType = { Race = "dungeon_rpg", Char = "dungeon_char", Train = "dungeon_train", }
...
DungeonEntryCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.ON_STAMINA_CHANGED] = 'OnStaminaChanged', [MessageConst.ON_SUB_GAME_UNLOCK] = "OnSubGameUnlock", }
...
DungeonEntryCtrl._OnBtnEnemyDetailsClick = HL.Method() << function(self)
    ...
    UIManager:AutoOpen(PanelId.CommonEnemyPopup, { title = dungeonTypeCfg.enemyInfoTitle, enemyIds = dungeonCfg.enemyIds, enemyLevels = dungeonCfg.enemyLevels })
end
...
DungeonEntryCtrl._RefreshContent = HL.Method() << function(self)
    ...
    local sprite = self:LoadSprite(UIConst.UI_SPRITE_DUNGEON, dungeonSeriesCfg.dungeonPicPath)
    if sprite then
        self.view.dungeonBG.sprite = sprite
    end
...
DungeonEntryCtrl._InitDungeonTabs = HL.Method() << function(self)
    ...
    if #dungeonTabInfos > 2 then
        ...
        self.view.dungeonSelectionNode:ScrollToRectTransform(targetScrollToCell.gameObject.transform, true)
    end
    self.view.dungeonSelectionNode.gameObject:SetActiveIfNecessary(dungeonSeriesCfg.gameCategory ~= GameMechanicsType.Char)
end
DungeonEntryCtrl._OnDungeonTabClick = HL.Method(HL.Any, HL.Number, HL.String) << function(self, cell, luaIndex, dungeonId)
    if self.m_selectedTabIndex == luaIndex then
        return
    end
...
DungeonEntryCtrl._RefreshDungeonDetailList = HL.Method() << function(self)
    ...
    self.view.timeTxt.text = isPass and UIUtils.getLeftTimeToSecondFull(subGameRecord.bestPassTime / 1000) or "--:--:--"
    self.view.recommendLvTxt.text = string.format("Lv.%s", dungeonCfg.recommendLv > 0 and tostring(dungeonCfg.recommendLv) or "-")
    self.view.dungeonDescTxt.text = UIUtils.resolveTextStyle(dungeonCfg.dungeonDesc)
    local hasFeature = DungeonUtils.isDungeonHasFeatureInfo(self.m_dungeonId)
    if hasFeature then
        self.view.ruleDescTxt.text = UIUtils.resolveTextStyle(dungeonCfg.featureDesc)
    end
    ...
    local complete = isTrain and dungeonMgr:IsDungeonRewardGained(self.m_dungeonId) or dungeonMgr:IsDungeonExtraRewardGained(self.m_dungeonId)
    self.m_dungeonGoalCellCache:Refresh(#goalTxtTbl, function(cell, luaIndex)
        cell.goalTxt.text = UIUtils.resolveTextStyle(goalTxtTbl[luaIndex])
...
DungeonEntryCtrl._ProcessDungeonRewards = HL.Method().Return(HL.Table) << function(self)
    ...
    local getRewardCommonInfo = function(itemBundle)
        ...
        return { id = itemBundle.id, count = itemBundle.count, sortId1 = itemCfg.sortId1, sortId2 = itemCfg.sortId2, }
    end
...
DungeonEntryCtrl._RefreshBottomStamina = HL.Method() << function(self)
    ...
    if costStamina > 0 then
        ...
        local color = cntStamina >= costStamina and self.view.config.STAMINA_ENOUGH_COLOR or self.view.config.STAMINA_NOT_ENOUGH_COLOR
        self.view.staminaCostDescTxt.color = color
        self.view.staminaCostNumberTxt.color = color
        self.view.staminaCostNumberTxt.text = costStamina
    end
end
