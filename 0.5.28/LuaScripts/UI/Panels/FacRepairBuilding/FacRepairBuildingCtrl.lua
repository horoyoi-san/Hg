...
FacRepairBuildingCtrl.s_messages = HL.StaticField(HL.Table) << { [MessageConst.FAC_ON_BUILDING_REPAIRED] = 'OnBuildingRepaired', }
...
FacRepairBuildingCtrl.OnCreate = HL.Override(HL.Any) << function(self, arg)
    ...
    local buildingData = { nodeId = nodeId, itemId = FactoryUtils.getBuildingItemId(buildingId) }
...
FacRepairBuildingCtrl.UpdateCount = HL.Method(HL.Boolean) << function(self, isInit)
    ...
    if isInit then
        ...
        self.m_costItemCache:Refresh(repairItems.Count, function(cell, index)
            ...
            local info = { id = bundle.id, count = bundle.count, }
