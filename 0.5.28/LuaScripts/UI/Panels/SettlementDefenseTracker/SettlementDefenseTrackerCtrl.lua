...
SettlementDefenseTrackerCtrl.OnCreate = HL.Override(HL.Any) << function(self, arg)
    ...
    self.m_xEdgeRadius = self.view.config.TRACKER_EDGE_RADIUS_X * (self.view.main.rect.width / STANDARD_HORIZONTAL_RESOLUTION)
    self.m_yEdgeRadius = self.view.config.TRACKER_EDGE_RADIUS_Y * (self.view.main.rect.height / STANDARD_VERTICAL_RESOLUTION)
...
SettlementDefenseTrackerCtrl._BuildTrackerData = HL.Method(Vector3, HL.Any).Return(HL.Table) << function(self, worldPos, tracker)
    return { worldPos = worldPos, tracker = tracker, isInBound = false, visible = true, }
end
...
SettlementDefenseTrackerCtrl._UpdateTrackersThread = HL.Method() << function(self)
    ...
    for _, spawnerData in ipairs(self.m_spawnerTrackerDataList) do
        if not UIUtils.isPosInScreen(spawnerData.worldPos, CameraManager.mainCamera) then
            spawnerData.visible = true
            self:_UpdateTrackerScreenPosition(spawnerData)
        else
            spawnerData.visible = false
        end
...
SettlementDefenseTrackerCtrl._UpdateTrackerScreenPosition = HL.Method(HL.Table) << function(self, data)
    ...
    local screenPos = UIUtils.objectPosToUI(data.worldPos, self.uiCamera)
