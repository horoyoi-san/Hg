...
DungeonUnlockConditionPopupCtrl._Refresh = HL.Method() << function(self)
    ...
    for _, conditionId in ipairs(uncompletedConditionIds) do
        ...
        local isKnown = true
        table.insert(uncompletedConditionInfo, { conditionId = conditionId, desc = isKnown and gameMechanicConditionCfg.desc, type = gameMechanicConditionCfg.conditionType:GetHashCode(), isKnown = isKnown, customSortId1 = isKnown and 1 or 0, })
    end
...
DungeonUnlockConditionPopupCtrl._UpdateCell = HL.Method(HL.Any, HL.Table) << function(self, cell, info)
    ...
    node.jumpIcon.gameObject:SetActiveIfNecessary(canJump)
