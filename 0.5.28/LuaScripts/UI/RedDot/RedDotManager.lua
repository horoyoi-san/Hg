...
RedDotManager.GetRedDotState = HL.Method(HL.String, HL.Opt(HL.Any, HL.Number)).Return(HL.Boolean, HL.Opt(HL.Number)) << function(self, name, arg, penetrateLevel)
    ...
    if redDot.Check then
        local succ
        succ, result, redDotType = xpcall(redDot.Check, debug.traceback, arg)
        if not succ then
    ...
    if redDot.sons then
        ...
        for k, v in pairs(redDot.sons) do
            if v then
                sonResult, sonRedDotType = self:GetRedDotState(k, arg, penetrateLevel)
                if sonResult and sonRedDotType >= penetrateLevel then
                    if sonRedDotType == RED_DOT_TYPE_MAX then
                        return sonResult, sonRedDotType
                    end
        ...
        for _, v in ipairs(list) do
            ...
            if isActive then
                if redDot.needArg then
                    ...
                    if result == nil then
                        result, redDotType = self:GetRedDotState(name)
                    end
                    v.onUpdate(result, redDotType)
                end
            else
...
RedDotManager.AddRedDotInstance = HL.Method(HL.String, HL.Function, HL.Forward('RedDot')).Return(HL.Opt(HL.Number)) << function(self, name, onUpdate, uiWidget)
    ...
    local redDotInstance = { redDotName = name, onUpdate = onUpdate, key = key, uiWidget = uiWidget, }
