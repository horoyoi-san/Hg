...
NaviGroupSwitcher.NaviGroupSwitcher = HL.Constructor(HL.Number, HL.Opt(HL.Table, HL.Boolean)) << function(self, parentGroupId, groupInfos, isReverse)
...
NaviGroupSwitcher.Move = HL.Method(HL.Boolean) << function(self, isNext)
    ...
    if info.beforeSwitch then
        info.beforeSwitch()
    end
    info.naviGroup:NaviToThisGroup()
end
NaviGroupSwitcher._GetGroupInfo = HL.Method(HL.Boolean).Return(HL.Opt(HL.Table)) << function(self, isNext)
    ...
    for k, info in ipairs(self.m_groupsInfos) do
        ...
        if NotNull(naviGroup) and naviGroup.IsTopLayer then
