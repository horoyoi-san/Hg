...
local ModuleType = { NoReturn = 1, ReturnTable = 2, ReturnUserData = 3, }
...
function string.toSnakeCase(str)
    ...
    local newStr = string.gsub(str:sub(2), "([%u%d]%l*)", function(ss)
        ss = "_" .. ss:lower()
        return ss
    end)
...
local RequireFailReason = { NoFile = 1, ReturnNotTable = 2, Else = 3, }
local function real_require_ex(moduleName, isReload, globalEnv)
    ...
    if not module then
        ...
        module = { name = moduleName, env = moduleEnv }
        if result then
